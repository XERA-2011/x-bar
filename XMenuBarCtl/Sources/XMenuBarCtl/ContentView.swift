...
struct ContentView: View {
        ...
        GroupBox("Quick Actions") {
            VStack(spacing: 6) {
                ...
                HStack {
                    ...
                    actionButton("Toggle Bar", action: "toggle-xmenubarbar")
                }
                HStack {
    ...
    private var setSection: some View {
        GroupBox("Set Setting (xmenubar://set)") {
            VStack(spacing: 6) {
                HStack {
                    Text("Key:")
    ...
    private var toggleSection: some View {
        GroupBox("Toggle Setting (xmenubar://toggle)") {
            VStack(spacing: 6) {
    ...
    private var getSection: some View {
        GroupBox("Get Setting (xmenubar://get → xmenubarctl://callback)") {
            VStack(spacing: 6) {
        ...
        GroupBox("Authorization") {
            VStack(spacing: 6) {
                Button("Request Authorization") { engine.sendAuthorize() }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                Text("Triggers whitelist dialog in XMenuBar")
                    .font(.caption2)
