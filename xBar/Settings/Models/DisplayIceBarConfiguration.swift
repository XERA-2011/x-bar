...
/// Per-display configuration for the XMenuBar Bar.
nonisolated struct DisplayIceBarConfiguration: Codable, Equatable {
    /// Whether the XMenuBar Bar is enabled on this display.
    let useIceBar: Bool

    /// Whether the always-hidden section alone opens in the XMenuBar Bar on this
    /// display, leaving the hidden section to expand inline as usual.
    ///
    /// Only applicable when ``useIceBar`` is `false`, which already routes
    /// every section to the XMenuBar Bar. Showing the always-hidden section inline
    ...
    let useXMenuBarBarForAlwaysHidden: Bool

    /// The location where the XMenuBar Bar appears on this display.
    ...
    /// The layout mode for the XMenuBar Bar on this display.
    let iceBarLayout: IceBarLayout

    /// The maximum number of items per row when the XMenuBar Bar is in grid layout.
    ///
    ...
    /// Returns a new configuration with the `useXMenuBarBarForAlwaysHidden` flag replaced.
    func withUseXMenuBarBarForAlwaysHidden(_ value: Bool) -> DisplayIceBarConfiguration {
        DisplayIceBarConfiguration(
            useIceBar: useIceBar,
            useXMenuBarBarForAlwaysHidden: value,
            iceBarLocation: iceBarLocation,
            alwaysShowHiddenItems: alwaysShowHiddenItems,
            iceBarLayout: iceBarLayout,
            gridColumns: gridColumns,
...
nonisolated extension DisplayIceBarConfiguration {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.useXMenuBarBarForAlwaysHidden = try container.decodeIfPresent(
            Bool.self,
            forKey: .useXMenuBarBarForAlwaysHidden
        ) ?? false
