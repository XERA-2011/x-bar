...
/// Manages per-display XMenuBar Bar configuration.
///
...
final class DisplaySettingsManager {
    ...
    /// Whether XMenuBar asks for confirmation before a spacing change relaunches
    ...
    /// Internal access so unit tests in XMenuBarTests can seed and assert it.
    ...
    /// connected display so XMenuBar does not overwrite a user's manual
    ...
    func handleExternalPerDisplaySettingsChange(_ notification: Notification) {
        ...
        case "useXMenuBarBarForAlwaysHidden":
            if notification.userInfo?["toggle"] as? Bool == true {
                if let uuid = specificUUID {
                    toggleUseXMenuBarBarForAlwaysHidden(forDisplayUUID: uuid)
                } else {
                    toggleUseXMenuBarBarForAlwaysHidden(scope: scope)
                }
            } else if let value = notification.userInfo?["value"] as? Bool {
                if let uuid = specificUUID {
                    setUseXMenuBarBarForAlwaysHidden(value, forDisplayUUID: uuid)
                } else {
                    setUseXMenuBarBarForAlwaysHidden(value, scope: scope)
                }
            }
    ...
    /// Sets useXMenuBarBarForAlwaysHidden for displays based on scope.
    private func setUseXMenuBarBarForAlwaysHidden(_ value: Bool, scope: SettingsURIHandler.PerDisplayScope) {
        if scope == .allNonIceBarDisplays {
            ...
            for screen in NSScreen.screens {
                ...
                if !config.useIceBar {
                    updateConfiguration(forDisplayUUID: uuid) { $0.withUseXMenuBarBarForAlwaysHidden(value) }
                }
            }
        } else {
            diagLog.debug("setUseXMenuBarBarForAlwaysHidden not implemented for scope \(scope)")
        }
    }

    /// Toggles useXMenuBarBarForAlwaysHidden for displays based on scope.
    private func toggleUseXMenuBarBarForAlwaysHidden(scope: SettingsURIHandler.PerDisplayScope) {
        if scope == .allNonIceBarDisplays {
            for screen in NSScreen.screens {
                guard let uuid = Bridging.getDisplayUUIDString(for: screen.displayID) else { continue }
                let config = configuration(forUUID: uuid)
                if !config.useIceBar {
                    updateConfiguration(forDisplayUUID: uuid) {
                        $0.withUseXMenuBarBarForAlwaysHidden(!$0.useXMenuBarBarForAlwaysHidden)
                    }
                }
            }
        } else {
            diagLog.debug("toggleUseXMenuBarBarForAlwaysHidden not implemented for scope \(scope)")
        }
    }

    /// Sets useXMenuBarBarForAlwaysHidden for a specific display UUID.
    private func setUseXMenuBarBarForAlwaysHidden(_ value: Bool, forDisplayUUID uuid: String) {
        updateConfiguration(forDisplayUUID: uuid) { config in
            config.withUseXMenuBarBarForAlwaysHidden(value)
        }
    }

    /// Toggles useXMenuBarBarForAlwaysHidden for a specific display UUID.
    private func toggleUseXMenuBarBarForAlwaysHidden(forDisplayUUID uuid: String) {
        let current = configuration(forUUID: uuid)
        updateConfiguration(forDisplayUUID: uuid) { config in
            config.withUseXMenuBarBarForAlwaysHidden(!current.useXMenuBarBarForAlwaysHidden)
        }
    }
    ...
    /// Whether the XMenuBar Bar is enabled for the given display.
    func useIceBar(for displayID: CGDirectDisplayID) -> Bool {
        configuration(for: displayID).useIceBar
    }

    /// Whether the always-hidden section alone opens in the XMenuBar Bar on the
    /// given display.
    func useXMenuBarBarForAlwaysHidden(for displayID: CGDirectDisplayID) -> Bool {
        configuration(for: displayID).useXMenuBarBarForAlwaysHidden
    }

    /// The XMenuBar Bar location for the given display.
    func iceBarLocation(for displayID: CGDirectDisplayID) -> IceBarLocation {
        configuration(for: displayID).iceBarLocation
    }

    /// The XMenuBar Bar layout for the given display.
    ...
    /// Whether any connected display has the XMenuBar Bar enabled.
    ...
    /// Toggles the XMenuBar Bar for the display with the active menu bar.
    func toggleIceBarForActiveDisplay() {
        guard let uuid = Bridging.getActiveMenuBarDisplayUUID() else {
            diagLog.warning("Cannot toggle XMenuBar Bar — no active menu bar display UUID")
            return
        }
        updateConfiguration(forDisplayUUID: uuid) { config in
