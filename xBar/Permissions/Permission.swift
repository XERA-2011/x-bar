...
class Permission: Identifiable {
    ...
    /// A Boolean value that indicates whether an OS permission prompt has already been requested.
    ...
    /// Whether this permission should open System Settings immediately on the first attempt,
    /// rather than waiting for a secondary request.
    private let openSettingsImmediately: Bool
    ...
    ///   - openSettingsImmediately: Whether to open System Settings immediately on the first request attempt.
    ...
    init(
        ...
        openSettingsImmediately: Bool = false,
        ...
        self.openSettingsImmediately = openSettingsImmediately
        self.openSettings = openSettings
        self.hasPermission = check()
        self.hasAttemptedRequest = self.hasPermission
    ...
    /// Performs the permission request.
    ///
    /// On the first attempt, the system permission prompt is requested so macOS can present its
    /// native permission dialog. If the permission is still missing on subsequent attempts (where
    /// macOS will not present the system dialog again), System Settings is opened directly.
    func performRequest() {
    ...
    /// Resets the request attempt state, allowing the next request to trigger the OS prompt
    /// rather than opening System Settings directly.
...
/// The Accessibility permission, required for xBar to detect, move, and
...
final class AccessibilityPermission: Permission {
    init() {
        super.init(
            ...
            request: {
                AXHelpers.isProcessTrusted(prompt: false)
            },
            openSettingsImmediately: true
        )
    }
}
