...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationWillFinishLaunching(_: Notification) {
        ...
        // Register xmenubar:// URL events early so external tools (e.g. Raycast)
        // can trigger actions even when XMenuBar is not currently in the foreground;
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        // On first launch, navigate the user directly to the Layout pane in Settings
        // where permissions can be granted without a separate popup window.
        if isFirstLaunch {
    ...
    /// Handles `kAEGetURL` Apple Events and forwards `xmenubar://` URLs to `handleURL(_:senderBundleId:)`.
    ...
    /// Dispatches an incoming `xmenubar://` URL to the appropriate action.
    ///
    /// Supported Action URLs:
    /// - `xmenubar://toggle-hidden` — toggle the hidden menu bar section
    /// - `xmenubar://toggle-always-hidden` — toggle the always-hidden menu bar section
    /// - `xmenubar://search` — open the menu bar item search panel
    /// - `xmenubar://toggle-xmenubarbar` — toggle the IceBar on the active display
    /// - `xmenubar://toggle-application-menus` — toggle application menus
    /// - `xmenubar://open-settings` — open the XMenuBar settings window
    ///
    /// Supported Settings URLs (requires whitelist authorization):
    /// - `xmenubar://set?key=X&value=Y` — set a boolean setting
    /// - `xmenubar://toggle?key=X` — toggle a boolean setting
    private func handleURL(_ url: URL, senderBundleId: String? = nil) {
        ...
        case .unrecognized:
            appState.diagLog.warning("Received unrecognized xmenubar:// URL: \(url.absoluteString)")
        }
    }

    /// Performs a parameterless `xbar://` or `xmenubar://` action.
    private func perform(_ action: SettingsURIAction) {
        switch action {
        case .toggleHidden:
            guard let section = appState.menuBarManager.section(withName: .hidden) else { return }
            section.toggle()
            if !section.isHidden {
                appState.menuBarManager.showOnHoverAllowed = false
            }
        case .toggleAlwaysHidden:
            guard let section = appState.menuBarManager.section(withName: .alwaysHidden) else { return }
            section.toggle()
            if !section.isHidden {
                appState.menuBarManager.showOnHoverAllowed = false
            }
        case .search:
            break
        case .toggleXBar, .toggleXMenuBarbar:
    ...
    /// Handles `xmenubar://get?key=X&callback=Y`.
