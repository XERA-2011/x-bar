...
nonisolated enum MarkerPairResolver {
    ...
    struct Marker: Equatable {
        ...
        /// resolves to a bundle ID that is not Control Center or XMenuBar.
        let owningPID: pid_t?
    }
    ...
    /// one unresolved icon. XMenuBar and Control Center are excluded from
    ...
    ///     items-only list. Callers are expected to pre-filter XMenuBar
    ///     control items and the XMenuBar self-registration window.
    ///   - xmenubarBundleID: XMenuBar's own bundle identifier; excluded from
    ...
    /// (bundle-identifier shape), is not a XMenuBar control item, and is
    /// not the XMenuBar self-registration window.
    static func extractMarkers(
        from windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)],
        xmenubarControlItemPrefix: String,
        xmenubarBundleID: String
    ) -> [Marker] {
        windows.compactMap { window in
            guard let title = window.title, title.contains(".") else { return nil }
            if title.hasPrefix(xmenubarControlItemPrefix) || title.hasPrefix("XMenuBar.ControlItem.") || title.hasPrefix("xBar.ControlItem.") {
                return nil
            }
            if title == xmenubarBundleID || title == "com.xera.xmenubar" || title == "com.xera.xbar" {
                return nil
            }
            return Marker(
