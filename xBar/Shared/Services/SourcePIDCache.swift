...
actor SourcePIDCache {
    ...
    private nonisolated func pidBody(for window: WindowInfo) -> pid_t? {
        ...
        let xmenubarBundleID = Bundle.main.bundleIdentifier ?? "com.xera.xbar"
        var appsChecked = 0
        var appsWithBar = 0
        var appsSkipped = 0
        ...
        for app in apps {
            ...
            autoreleasepool {
                ...
                appsWithBar += 1
                // XMenuBar's own children are never skipped for being disabled.
                ...
                // skipping it here leaves XMenuBar unable to resolve its own
                ...
                // XMenuBar then cannot identify its own dividers even with an
                // exact positional match available (#899, and the
                // "strategies 1 through 3 never fired" report in #895).
                let isOwnApp = app.bundleIdentifier == xmenubarBundleID || app.bundleIdentifier == "com.xera.xmenubar"
        ...
        //
        // XMenuBar and Control Center are excluded: attributing a widget to
        ...
        let attributableBundleIDs = apps.compactMap { app -> String? in
            guard let bundleID = app.bundleIdentifier,
                  bundleID != xmenubarBundleID,
                  bundleID != "com.xera.xmenubar",
                  bundleID != ccBundleID
            else {
                return nil
            }
        ...
        // CG-layer owning PID (when it is neither XMenuBar itself nor
        // Control Center) or by looking up the running app named by
        // the marker's bundle-ID title. Multi-match cases are skipped
        // to prevent misattribution. XMenuBar's own control items and
        // self-registration windows are excluded so XMenuBar's PID can
        ...
        if !unresolvedWindows.isEmpty {
            let markers = MarkerPairResolver.extractMarkers(
                ...
                xmenubarControlItemPrefix: "xBar.ControlItem.",
