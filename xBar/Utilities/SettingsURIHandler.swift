...
/// Handles settings manipulation via xmenubar:// URLs with whitelist-based security.
@MainActor
enum SettingsURIHandler {
    ...
    /// Handles xmenubar://set?key=X&value=Y&type=bool URL.
    ...
    private static func handleDoubleSet(key: String, value: String) -> Bool {
        ...
        let valueToStore = Swift.max(minVal, Swift.min(doubleValue, maxVal))
    ...
    private static func handlePerDisplaySet(key: String, value: String, displayUUID: String?) -> Bool {
        ...
        case "useXMenuBarBarForAlwaysHidden":
            guard let boolValue = parseBool(value) else {
                diagLog.warning("Settings URI: Invalid boolean value '\(value)' for useXMenuBarBarForAlwaysHidden")
                return false
            }
            ...
            diagLog.info("Settings URI: Set useXMenuBarBarForAlwaysHidden = \(boolValue) on all non-IceBar displays")
            return true
    ...
    private static func handlePerDisplaySetForSpecificDisplay(key: String, value: String, displayUUID: String) -> Bool {
        ...
        case "useXMenuBarBarForAlwaysHidden":
            ...
            // Post notification for specific display
            postPerDisplaySettingsDidChangeNotification(key: key, value: boolValue, scope: .specificDisplay(uuid: displayUUID))
            diagLog.info("Settings URI: Set useXMenuBarBarForAlwaysHidden = \(boolValue) on display \(displayUUID)")
            return true
    ...
    /// Handles xmenubar://toggle?key=X URL.
    ...
    /// Currently only supports useIceBar, useXMenuBarBarForAlwaysHidden and alwaysShowHiddenItems.
    private static func handlePerDisplayToggle(key: String, displayUUID: String?) -> Bool {
        ...
        if let uuid = displayUUID, !uuid.isEmpty {
            ...
            case "useXMenuBarBarForAlwaysHidden":
                postPerDisplaySettingsDidChangeNotification(key: key, toggle: true, scope: .specificDisplay(uuid: uuid))
                diagLog.info("Settings URI: Toggled useXMenuBarBarForAlwaysHidden on display \(uuid)")
                return true
        ...
        case "useXMenuBarBarForAlwaysHidden":
            ...
            diagLog.info("Settings URI: Toggled useXMenuBarBarForAlwaysHidden on all non-IceBar displays")
            return true
    ...
    /// Handles xmenubar://get?key=X&callback=Y URLs.
    ...
    /// `validValues` maps a `xmenubar://get` advertises. The function is a read
    ...
    static func getSettingValue(key: String, displayUUID: String?) -> [String: Any]? {
        ...
        if perDisplayKeys.contains(key) {
            ...
            case "useXMenuBarBarForAlwaysHidden":
                return [
                    "value": config.useXMenuBarBarForAlwaysHidden,
                    "type": "boolean",
                ]
            case "iceBarLocation":
                return [
    ...
    private static func getDisplayInfo(screen: NSScreen, uuid: String) -> [String: Any] {
        ...
        return [
            ...
            "useXMenuBarBarForAlwaysHidden": config.useXMenuBarBarForAlwaysHidden,
...
extension Notification.Name {
    ...
    static let settingsDidChangeViaURI = Notification.Name("com.xera.xbar.settingsDidChangeViaURI")

    /// Posted when a per-display setting is changed externally via Settings URI scheme.
    static let perDisplaySettingsDidChangeViaURI = Notification.Name("com.xera.xbar.perDisplaySettingsDidChangeViaURI")

    /// Posted when a get request response is broadcast via distributed notification.
    static let settingsURIGetResponse = Notification.Name("com.xera.xbar.settingsURIGetResponse")

    /// Posted when the Settings URI whitelist changes.
    static let settingsURIWhitelistDidChange = Notification.Name("com.xera.xbar.settingsURIWhitelistDidChange")
}
