...
nonisolated enum ScreenCapture {
    ...
    static func cachedCheckPermissions(reset: Bool = false) -> Bool {
        enum Context {
            static let state = OSAllocatedUnfairLock<(result: Bool, timestamp: ContinuousClock.Instant)?>(initialState: nil)
        }
        let now = ContinuousClock.now
        if !reset, let current = Context.state.withLock({ $0 }) {
            if current.result {
                return true
            }
            if now - current.timestamp < .seconds(1) {
                return false
            }
        }
        let result = checkPermissions()
        diagLog.debug("cachedCheckPermissions: computed fresh result = \(result) (reset=\(reset))")
        Context.state.withLock { $0 = (result, now) }
        return result
    }
...
private final nonisolated class FrameCaptor: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    static let sampleHandlerQueue = DispatchQueue(label: "com.xera.xbar.screencapture")
