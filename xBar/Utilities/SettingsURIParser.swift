...
/// A parameterless action reachable via `xmenubar://`.
///
/// Explicitly `nonisolated`: the XMenuBar target defaults to MainActor isolation,
...
nonisolated enum SettingsURIAction: String, CaseIterable, Equatable, Sendable {
    ...
    case search
    case toggleXBar = "toggle-xbar"
    case toggleXMenuBarbar = "toggle-xmenubarbar"
    case toggleApplicationMenus = "toggle-application-menus"
    case openSettings = "open-settings"
}

/// The decoded intent of an incoming `xmenubar://` URL.
...
nonisolated struct SettingsURIRequest: Equatable, Sendable {
    ...
    /// `xmenubar://get?key=version` is read-only metadata and skips authorization.
...
/// Decomposes `xmenubar://` URLs into ``SettingsURIRequest`` values.
///
