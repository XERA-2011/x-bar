...
extension HIDEventManager {
    ...
    private func handleShowOnClick(appState: AppState, screen: NSScreen, clickLocation: CGPoint, modifierFlags: NSEvent.ModifierFlags, isDoubleClick: Bool = false) {
        ...
        // on a widget's icon don't also trigger XMenuBar's show-on-click reveal.
        ...
        if isDoubleClick {
            ...
            {
                // If the always-hidden section is currently showing via the XMenuBar
                // Bar, a plain click should close it rather than switch the XMenuBar
    ...
    private func handleSmartRehide(
        ...
        // Only continue if the click is not inside the XMenuBar Bar, at
    ...
    /// using the system-wide accessibility hit-test. Excludes XMenuBar's own
    ...
    /// role). When the helper returns true, XMenuBar defers to the widget under
    ...
    /// Returns whether any non-XMenuBar window at the pop-up-menu window level is
    ...
    /// overlay, which is needed to avoid showing XMenuBar's secondary context
    ...
    ///
    /// After a profile change with XMenuBarBar active, the Window Server tracks
    ...
    private func handleShowOnHover(appState: AppState, screen: NSScreen) {
        ...
        if hiddenSection.isHidden {
            ...
            let taskToken = UUID()
            hoverTaskToken = taskToken
            // When the XMenuBar Bar (Ice Bar) is the active presentation, the
...
extension HIDEventManager {
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the XMenuBar Bar panel.
    func isMouseInsideIceBar(appState: AppState) -> Bool {
        ...
        // moves their mouse outside of the XMenuBar Bar.
