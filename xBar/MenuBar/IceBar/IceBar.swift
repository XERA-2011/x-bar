...
final class IceBarPanel: NSPanel {
    ...
    /// Manager for the XMenuBar Bar's color.
    ...
    /// Creates a new XMenuBar Bar panel with Liquid Glass support.
    init() {
        super.init(
            contentRect: .zero,
...
private struct IceBarContentView: View {
    ...
    /// Determines whether the IceBar should render colorful app icons instead of menu bar glyphs.
    private var prefersAppIcon: Bool {
        if !ScreenCapture.cachedCheckPermissions() {
            return true
        }
        return appState.settings.advanced.alwaysUseAppIconForMenuBarItems
    }
