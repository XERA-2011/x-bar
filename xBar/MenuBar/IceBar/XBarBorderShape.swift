...
/// The xBar Bar's rounded-rectangle geometry, serving as both its clip and
/// its border stroke.
///
...
nonisolated struct XBarBorderShape: InsettableShape {
    ...
    /// The xBar Bar's clip for a given content height: fully rounded ends get
    /// a circular half-height radius, square corners a continuous
    /// quarter-height one.
    static func xbarBarClip(height: CGFloat, hasRoundedShape: Bool) -> XBarBorderShape {
        XBarBorderShape(
    ...
    /// Legacy alias for ``xbarBarClip(height:hasRoundedShape:)``.
    static func xmenubarBarClip(height: CGFloat, hasRoundedShape: Bool) -> XBarBorderShape {
        xbarBarClip(height: height, hasRoundedShape: hasRoundedShape)
    }

    /// The border that traces ``xbarBarClip(height:hasRoundedShape:)``.
    ///
    ...
    static func xbarBarBorder(
        height: CGFloat,
        hasRoundedShape: Bool,
        borderWidth: CGFloat
    ) -> XBarBorderShape {
        var shape = xbarBarClip(height: height, hasRoundedShape: hasRoundedShape)
        shape.omitTopEdge = !hasRoundedShape
        return shape.inset(by: borderWidth / 2)
    }

    /// Legacy alias for ``xbarBarBorder(height:hasRoundedShape:borderWidth:)``.
    static func xmenubarBarBorder(
        height: CGFloat,
        hasRoundedShape: Bool,
        borderWidth: CGFloat
    ) -> XBarBorderShape {
        xbarBarBorder(height: height, hasRoundedShape: hasRoundedShape, borderWidth: borderWidth)
    }

    func inset(by amount: CGFloat) -> XBarBorderShape {
        var shape = self
