...
final class LayoutBarPaddingView: NSView {
    ...
    override func draggingExited(_ sender: NSDraggingInfo?) {
        ...
        // destination. Each entered row is frozen above, so xmenubar a row as
    ...
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
           ...
           container.section != .visible
        {
            ...
            // starts no move task that would xmenubar it later. Resume it like
    ...
    private func move(
        ...
        stabilizationTask = Task { [self, appState] in
            ...
            let watchdogTask = Task { [weak self, weak appState, weak sourceContainer, revealedSections] in
                ...
                // run. The reset above xmenubared the bars; park the divider
            ...
            if !didRefresh {
                Self.diagLog.error(
                    "XMenuBaring Layout editor after post-move cache refresh timed out"
                )
            }
            let didXMenuBarCurrentMove = await MainActor.run {
    ...
    private func resetStabilizingStateIfNeeded(sourceContainer: LayoutBarContainer? = nil) async {
        if isStabilizing {
            ...
            // exit has to xmenubar it too or it stays at its mid-drag snapshot
    ...
    private func revealEmptySectionDivider(
        ...
        // reveal is a XMenuBar-internal detour, so every cancellation exit
