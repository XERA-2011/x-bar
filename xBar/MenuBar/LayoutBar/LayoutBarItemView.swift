...
final class LayoutBarItemView: LayoutBarArrangedView {
    ...
    /// Determines whether the item view should draw the app icon rather than the captured glyph.
    private var prefersAppIcon: Bool {
        if !ScreenCapture.cachedCheckPermissions() {
            return true
        }
        return appState?.settings.advanced.alwaysUseAppIconForMenuBarItems ?? false
    }
    ...
    /// the last stable thumbnail until the container xmenubars.
    ...
    private var isTriggerControlled: Bool { false }
    ...
    override func mouseEntered(with event: NSEvent) {
        ...
        tooltipController.text = effectiveItem.displayName
        ...
        if let appState {
            ...
            appIconPreferenceObservationTask = Task { @MainActor [weak self, weak appState] in
                ...
                for await _ in changes {
                    ...
                    let prefers = self.prefersAppIcon
                    guard prefers != previous else { continue }
                    previous = prefers
    ...
    /// the static system items; an unresolved Control Center slot is XMenuBar's
    ...
    private func aliasForUnresolvedControlCenterPlaceholder(
        ...
        guard let bundleID = UnresolvedPlaceholderAlias.appBundleID(
            ...
            xmenubarBundleID: Constants.bundleIdentifier
        ) else {
            Self.diagLog.warning(
    ...
    /// XMenuBar Bar substitute the same image for the same item, and so both
    ...
    private static func makePlaceholderImage(for item: MenuBarItem) -> (NSImage?, Bool) {
        ...
        return (
            MenuBarItemIconFallback.fallbackSymbolImage(for: item),
            false
        )
    }
