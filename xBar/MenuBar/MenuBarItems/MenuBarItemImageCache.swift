...
final class MenuBarItemImageCache: @unchecked Sendable {
    ...
    /// Default refresh interval (4 fps), fixed for power efficiency and smooth previews.
    static nonisolated let defaultIconRefreshInterval: TimeInterval = 0.25
    ...
    private static var cacheFileURL: URL? {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.xera.xbar"
        return cacheDir?.appendingPathComponent("\(bundleID)/imageCache.json")
    }
    ...
    private func configureCancellables() {
        ...
        if let appState {
            ...
            navigationStateObservationTask = Task { @MainActor [weak self, appState] in
                let changes = Observations {
                    (
                        ...
                        appState.settings.general.operationMode
                    )
                }
    ...
    private func hasVisibleCaptureConsumer() -> Bool {
        guard appState != nil, ScreenCapture.cachedCheckPermissions() else {
            return false
        }
        ...
        Task { [weak self] in
            ...
            if needsRefresh {
                ...
                MenuBarItemImageCache.diagLog.debug(
                    "Starting live refresh (iceBar=\(nav.isIceBarPresented), settings=\(nav.isSettingsPresented))"
                )
    ...
    private func runLiveRefreshLoop() async {
        ...
        while !Task.isCancelled {
            ...
            let interval = Self.defaultIconRefreshInterval
    ...
    private nonisolated func compositeCapture(
        ...
        // The capture backend picks the pixel scale, not XMenuBar: SCK captures
    ...
    ///
    /// `expected` is the scale of the display XMenuBar resolved for the menu
    ...
    private nonisolated func individualCapture(
        ...
        for item in items {
            ...
            // `scale` comes from the display XMenuBar believes owns the menu
    ...
    private nonisolated func captureImages(
        ...
        // XMenuBar's own control items always capture as transparent via
