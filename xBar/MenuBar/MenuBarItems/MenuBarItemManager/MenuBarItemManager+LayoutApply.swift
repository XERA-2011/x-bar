...
extension MenuBarItemManager {
    ...
    /// movable, hideable items (except the XMenuBar icon) to the
    /// Hidden section.
    ///
    ...
    /// Moves every movable, hideable item except the XMenuBar icon to Visible.
    func resetLayoutToVisible() async throws -> Int {
        try await resetLayout(to: .visible)
    }

    /// Moves every movable, hideable item except the XMenuBar icon to Always Hidden.
        ...
        func movePass(_ items: [MenuBarItem], controls: ControlItemPair) async -> Int {
            ...
            for item in items {
                if item.tag == .visibleControlItem {
                    continue // Keep the XMenuBar icon in the visible section if enabled.
                }
    ...
    /// quit apps still appear in Settings Layout, XMenuBarBar, and Search
    ...
    func applyProfileLayout(
        ...
        // something XMenuBar decided to do, and it can afford to wait for a
        ...
        // PID are widgets macOS hosts under Control Center that XMenuBar cannot yet
        ...
        // items that won't fit into the hidden section. The XMenuBar visible
            ...
            let notch = screen.frameOfNotch
        {
            ...
            // Find the XMenuBar visible control icon, which must always stay visible.
            let visibleCtrlUID = items.first(where: { $0.tag == .visibleControlItem })?.uniqueIdentifier
        ...
        // XMenuBar's own items first, or a collapsed bar always looks like it
        ...
        if hiddenBoundaryMismatch > 0, canRepositionControlItems, !Task.isCancelled {
            ...
            if shouldMoveHiddenDivider {
                ...
                // XMenuBar's own control items clear both filters above whatever the
           ...
           ahCtrlUID != nil
        {
            ...
            // visible/hidden boundary repair. Same reasoning as the XMenuBar-icon
        ...
        // XMenuBar's own dividers is what walks it across the bar (#924, #927).
    ...
    /// That value describes a bar XMenuBar has never arranged. Writing it onto a
    ...
    /// - `activelyShownTags` carries the items XMenuBar is temporarily showing. One
    ///   of those sits outside its saved section because XMenuBar put it there, and
    ...
    func applySavedLayout(
        ...
        // Each guard logs a distinct reason so a "XMenuBar stopped
