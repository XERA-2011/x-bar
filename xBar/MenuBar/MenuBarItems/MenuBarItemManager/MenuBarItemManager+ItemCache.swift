...
extension MenuBarItemManager {
    ...
    struct ControlItemPair {
        ...
        init?(
            ...
            // XMenuBar instances can produce identical titles; tag assignment then
            ...
            // correlation guessing at XMenuBar's own dividers (#923, #924, #927).
            //
            // XMenuBar holds these windows, so it does not have to guess. Only
            ...
            // XMenuBar's own AX elements. XMenuBar's control items are its own
            // NSStatusItems, so their AX elements (reached via XMenuBar's own
            ...
            // the only strategy that lets XMenuBar identify its OWN control
        ...
        /// Whether to rebuild one of XMenuBar's own control items directly from
        ...
        /// claimed.
        ///
        /// With an authoritative window ID — XMenuBar's own `NSStatusItem` window
        ...
        /// duplicate XMenuBar instance. Without an authoritative ID, the
        ...
        /// Strategy 4: correlates XMenuBar's own AX element frames (from its own
        ...
        private static func matchViaAXFrame(
            ...
            guard
                ...
                MenuBarItemManager.diagLog.debug(
                    "ControlItemPair: strategy 4 (AX frame) unavailable — could not resolve XMenuBar's own extrasMenuBar"
                )
                return nil
            }
            ...
            guard !snapshot.isEmpty else {
                MenuBarItemManager.diagLog.debug(
                    "ControlItemPair: strategy 4 (AX frame) unavailable — XMenuBar's extrasMenuBar has no children with frames"
                )
                return nil
            }
        ...
        struct CandidateFrame {
            ...
            /// Whether this candidate is XMenuBar's *visible* control item, which
        ...
        /// order XMenuBar's own status items are enumerated in), so the first
    ...
    ///
    /// Control Center can outlive the XMenuBar process whose status item it
    /// hosted and keep serving that window. #1032's reporter carried one —
    /// `com.xera.xmenubar:com.xera.xmenubar`, window 639 — across relaunches
    ...
    static nonisolated func orphanedOwnNamespaceWindowIDs(
        ...
        return Set(
            items.lazy
                .filter { item in
                    (item.tag.namespace == .xbar || item.tag.namespace == .xmenubar)
    ...
    /// re-hosting status items, not that XMenuBar should recreate them.
    ...
    /// Whether bundleID owns a menu bar item XMenuBar already tracks: an entry
    ...
    func cacheItemsRegardless(
        ...
        // A duplicate XMenuBar process (or windows left by one that crashed) can
        ...
        // A window Control Center kept serving for a XMenuBar process that is
        ...
        guard !isInStartupSettling else {
            ...
            // relocateXMenuBarIcon moves our own control item within the first
            ...
            // stamped at 16.375 by relocateXMenuBarIcon, which left the reporter
    ...
    /// editor may xmenubar its source and destination rows after a user move.
    ///
    /// Ordinary background refreshes deliberately drop when ``CacheGate`` is
    /// busy. An editor move cannot: xmenubaring from the old cache duplicates or
    ...
    private func recacheIfSourceProcessesResolved(_ itemWindowIDs: [CGWindowID]) async {
        ...
        // this side can see and a cached item cannot: a duplicate XMenuBar process
        // can leave control-item windows behind under foreign window IDs.
        let windows = WindowInfo.createWindows(from: probeWindowIDs)
            .filter { !($0.title?.hasPrefix("XMenuBar.ControlItem.") ?? false) }
