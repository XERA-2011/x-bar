...
final class MenuBarItemManager {
    ...
    /// When the user last moved an item themselves, as opposed to XMenuBar
    ...
    /// whoever moved it. The save gate needs to tell them apart. XMenuBar's own
    ...
    /// throughout. When the anchor is one of XMenuBar's own dividers, repeating
    ...
    ///     (XMenuBar chevron); its position within the visible section is
    ...
    func computeSectionOrder(from cache: ItemCache) -> [String: [String]] {
        ...
        // are the typical case. The visibleControlItem (XMenuBar chevron) is
        // also persisted so its user-chosen position within the visible
        // section survives XMenuBar restarts: without it, savedSectionOrder
    ...
    /// arrangement XMenuBar generated itself, letting the next cache cycle
    /// persist it.
    ///
    /// Pure over its inputs.
