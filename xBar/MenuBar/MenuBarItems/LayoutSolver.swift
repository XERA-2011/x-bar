...
nonisolated enum LayoutSolver {
    ...
    enum LeftmostMove: Equatable {
        /// The XMenuBar visible-control icon is sitting left of the hidden
        /// divider; restore it to the visible section.
        case xmenubarIcon(MenuBarItem)
    ...
    /// of the three XMenuBar control items.
    ///
    ...
    /// that drag the XMenuBar icon to the user's configured anchor on every
    /// cache cycle. Visible-control-item exclusion was the omission
    /// that caused the field-reported "XMenuBar icon keeps moving" bug.
    ///
    ...
    /// hosts under Control Center that XMenuBar cannot yet attribute to their
    ...
    /// (1) XMenuBar visible-control icon recovery, (2) non-hideable system
    ...
    /// stable choice. The XMenuBar icon is a control item but must always be
    ...
    /// The XMenuBar-icon relocation decision on its own, for callers that must
    ...
    static nonisolated func planXMenuBarIconMove(
        items: [MenuBarItem],
        hiddenBounds: CGRect
    ) -> MenuBarItem? {
    ...
    static nonisolated func planLeftmostMove(
        ...
        // Path 1: XMenuBar icon.
        if let xmenubarIcon = leftmostItems.first(where: { $0.tag == .visibleControlItem }) {
            return .xmenubarIcon(xmenubarIcon)
        }
    ...
    /// mis-positions the control items (the XMenuBar visible icon jumps to the far
    ...
    static nonisolated func planNotchOverflow(
        ...
        // always-visible XMenuBar icon to the leftmost slot on every overflow even
    ...
    ///
    /// XMenuBar's own control items are what reaches that test. The caller's
    ...
    /// with XMenuBar quit and watched the first apply after relaunch collapse
    ...
    /// always-hidden sections are revealed through the XMenuBar Bar, which
    ...
    static nonisolated func planLCSMoveSequence(
        ...
        for uid in itemsToMove {
            ...
            //
            // Skips anchors the caller marked unanchorable — XMenuBar's own
    ...
    /// Whether an identifier claims XMenuBar's own namespace while naming an item
    /// XMenuBar does not own.
    ///
    ...
    private static nonisolated func isForeignEntryUnderOwnNamespace(identifier: String) -> Bool {
        let ns = namespace(forIdentifier: identifier)
        guard ns == MenuBarItemTag.Namespace.xbar.description || ns == MenuBarItemTag.Namespace.xmenubar.description else {
            return false
        }
    ...
    /// bundle identifier — for the whole bar at once, XMenuBar's own control
    ...
    ///
    /// - **A control item lost its name.** XMenuBar titles its own items
    ///   `XMenuBar.ControlItem.*`, so one in our namespace titled with our bundle
    ...
    static nonisolated func liveIdentitiesAreDegraded(
        ...
        let own = MenuBarItemTag.Namespace.xbar.description
        ...
        if identities.contains(where: { ($0.namespace == own || $0.namespace == MenuBarItemTag.Namespace.xmenubar.description) && isSelfTitled($0) }) {
            return true
        }
    ...
    /// occasionally hands a foreign window XMenuBar's own PID, and the layout then
    /// holds e.g. `com.xera.xmenubar:WiFi` for an item Control Center owns.
    ...
    static nonisolated func prunedSectionOrder(
        ...
        // and kept `com.xera.xmenubar:WiFi`, so the live WiFi item was planned
        ...
        return savedSectionOrder.reduce(into: [String: [String]]()) { result, entry in
            ...
            result[sectionKey] = identifiers.filter { identifier in
                ...
                // its title, XMenuBar's own control items by their reserved
                ...
                if isDisplayNameNamespace(identifier: identifier, aliases: displayNameAliases) {
                    ...
                    if title.hasPrefix("XMenuBar.ControlItem.") || title.contains(".Spacer.") {
                        return false
                    }
    ...
    /// clear them. The gate's own effect preserves its trigger, so XMenuBar goes
