...
nonisolated struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    var isSystemItem: Bool {
        switch namespace {
        case .controlCenter, .systemUIServer, .textInputMenuAgent, .weather, .passwords, .screenCaptureUI, .ssMenuAgent, .xbar, .xmenubar, .gamePolicyAgent:
            return true
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .xbar, title: identifier.rawValue, instanceIndex: 0)
    }
}
...
nonisolated extension MenuBarItemTag.Namespace {
    /// The namespace for the "xBar" process.
    static let xbar = string(Constants.bundleIdentifier)

    /// Legacy namespace alias for the app process.
    static let xmenubar = xbar
