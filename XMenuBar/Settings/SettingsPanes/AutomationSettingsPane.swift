...
struct AutomationSettingsPane: View {
    ...
    private var enableSection: some View {
        IceSection {
            Toggle("Enable Settings URI Scheme", isOn: $settings.isSettingsURIEnabled)
                .annotation("Allow external applications to read and modify \(Constants.displayName) settings via xmenubar:// URLs.")
    ...
    private var aboutSection: some View {
        IceSection("How It Works") {
            VStack(alignment: .leading, spacing: 8) {
                numberedStep(1, "When an app sends a xmenubar:// URL to change settings, XMenuBar checks if that app is whitelisted.")
    ...
    private var envVarsSection: some View {
        IceSection("Script Environment") {
            ...
            Text(verbatim: "XMENUBAR_HOOK_PHASE, XMENUBAR_HOOK_SCOPE, XMENUBAR_PROFILE_ID, XMENUBAR_PROFILE_NAME, XMENUBAR_PREVIOUS_PROFILE_ID, XMENUBAR_PREVIOUS_PROFILE_NAME")
                .font(.caption.monospaced())
                .foregroundStyle(.secondary)
                .textSelection(.enabled)
                .fixedSize(horizontal: false, vertical: true)

            Text("Example: a bash pre-hook could `defaults write com.bjango.istatmenus5 ActiveProfile -string \"$XMENUBAR_PROFILE_NAME\"` to keep iStat Menus in sync with XMenuBar.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .textSelection(.enabled)
                .fixedSize(horizontal: false, vertical: true)
