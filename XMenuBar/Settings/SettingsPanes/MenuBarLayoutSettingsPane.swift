...
struct MenuBarLayoutSettingsPane: View {
    ...
    private var useXMenuBarBarOnNotchOverflow: some View {
        Toggle(
            "Use the XMenuBar Bar while items are overflowed",
            isOn: $advancedSettings.useXMenuBarBarOnNotchOverflow
        )
        .annotation {
            Text(
                """
                Reveal hidden items through the XMenuBar Bar while overflow has items \
                ...
                Disable to always follow the per-display XMenuBar Bar setting.
                """
            )
            .padding(.trailing, 75)
        }
    ...
    private var iconRefreshInterval: some View {
        ...
        .annotation("How often animated icons refresh in the visible section, Hidden XMenuBar Bar, Search, and Layout. Always Hidden is capped at 1 fps. Higher values use more CPU.")
    }
