...
struct DisplaySettingsPane: View {
    ...
    private var confirmSpacingRelaunchControls: some View {
        Toggle("Confirm before relaunching apps", isOn: $displaySettings.confirmSpacingRelaunch)
            .annotation("Before a display change or spacing edit relaunches your menu bar apps, XMenuBar asks you to confirm. Turn this off to apply spacing changes and relaunch apps without confirmation.")
    ...
    /// the identical block once per display (xmenubar-next's redesign).
    @ViewBuilder
    ...
    private func displayRow(for display: DisplaySettingsManager.DisplayInfo) -> some View {
        ...
        let useXMenuBarBarForAlwaysHidden = Binding<Bool>(
            get: { displaySettings.configuration(forUUID: display.id).useXMenuBarBarForAlwaysHidden },
            set: { newValue in
                displaySettings.updateConfiguration(forDisplayUUID: display.id) { config in
                    config.withUseXMenuBarBarForAlwaysHidden(newValue)
                }
            }
        )
        ...
        IceBarConfigurationControls(
            ...
            useIceBar: useIceBar,
            useXMenuBarBarForAlwaysHidden: useXMenuBarBarForAlwaysHidden,
            location: location,
            layout: layout,
    ...
    private func globalSection() -> some View {
        ...
        let useXMenuBarBarForAlwaysHidden = Binding<Bool>(
            get: { displaySettings.globalConfiguration.useXMenuBarBarForAlwaysHidden },
            set: {
                displaySettings.globalConfiguration = displaySettings.globalConfiguration
                    .withUseXMenuBarBarForAlwaysHidden($0)
            }
        )
        let location = Binding<IceBarLocation>(
    ...
    private func globalConfirmationMessage(for pending: PendingGlobalApply) -> String {
        let profileName = pending.activeProfileName ?? ""
        let displayMessage = String(localized: "This will overwrite the settings of \(pending.displayCount) displays with the global template. If the active display's spacing changes, XMenuBar will relaunch each app with a menu bar item. Relaunching apps may cause unsaved input, progress, or transient app state to be lost.")
        if pending.activeProfileID != nil {
...
private struct IceBarConfigurationControls<ExtraControls: View>: View {
    ...
    @Binding var useXMenuBarBarForAlwaysHidden: Bool
    ...
    /// Whether anything opens in the XMenuBar Bar, and so whether its appearance
    /// controls apply.
    private var showsXMenuBarBar: Bool {
        useIceBar || useXMenuBarBarForAlwaysHidden
    }

    init(
        alwaysShowHiddenItems: Binding<Bool>,
        useIceBar: Binding<Bool>,
        useXMenuBarBarForAlwaysHidden: Binding<Bool>,
        ...
        _useXMenuBarBarForAlwaysHidden = useXMenuBarBarForAlwaysHidden
        _location = location
        _layout = layout
    ...
    var body: some View {
        ...
        Toggle("Always-hidden items only", isOn: $useXMenuBarBarForAlwaysHidden)
            .disabled(useIceBar)
            .annotation {
                if useIceBar {
