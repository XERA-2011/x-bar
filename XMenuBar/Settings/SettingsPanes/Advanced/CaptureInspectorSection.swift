...
/// Shows the untreated frame XMenuBar captures, so granting Screen Recording comes
/// with the ability to confirm the scope of what is observed.
///
/// Ported (in reduced form) from xmenubar-next: that version reads the frames back
/// out of the dedicated XMenuBarCapture package; this one drives the app's own
...
///
/// The copy has to state the scope exactly: XMenuBar reads a band across the top of
...
struct CaptureInspectorSection: View {
    ...
    private var scopeDescription: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("XMenuBar reads a band across the top of your display — the menu bar and its background. That band can contain the frontmost app's menus.")
                .font(.footnote)
