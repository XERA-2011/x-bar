...
private extension UTType {
    static let xmenubarTriggerPriority = UTType(exportedAs: "com.xera.xmenubar.trigger-priority")
}
...
struct TriggersSettingsPane: View {
    ...
    private func dragProvider(for triggerID: UUID) -> NSItemProvider {
        ...
        return NSItemProvider(item: data, typeIdentifier: UTType.xmenubarTriggerPriority.identifier)
    }
...
private struct TriggerPriorityDropDelegate: DropDelegate {
    ...
    func dropEntered(info: DropInfo) {
        guard info.hasItemsConforming(to: [.xmenubarTriggerPriority]) else { return }
        guard let draggedTriggerID, draggedTriggerID != targetID else { return }
        updateDropIndicator(for: draggedTriggerID)
    }

    func dropUpdated(info: DropInfo) -> DropProposal? {
        guard info.hasItemsConforming(to: [.xmenubarTriggerPriority]) else {
    ...
    func performDrop(info: DropInfo) -> Bool {
        guard info.hasItemsConforming(to: [.xmenubarTriggerPriority]) else { return false }
        defer {
            draggedTriggerID = nil
            dropIndicator = nil
        }
