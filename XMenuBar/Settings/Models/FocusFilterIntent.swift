...
/// An entity representing a XMenuBar profile, exposed to the system
...
struct ProfileEntityQuery: EntityQuery {
    ...
    private func allProfiles() -> [ProfileEntity] {
        // The path lives on XMenuBarFocusModeStore so the two readers of this
        // manifest cannot drift apart.
        guard
            let manifestURL = XMenuBarFocusModeStore.manifestURL,
...
/// Focus Filter that applies a XMenuBar profile when a Focus mode activates.
/// Appears in System Settings → Focus → [mode] → Focus Filters → Add "XMenuBar".
struct XMenuBarFocusFilter: SetFocusFilterIntent {
    ...
    static nonisolated(unsafe) var description: IntentDescription? = IntentDescription(
        "Apply a XMenuBar menu bar profile when this Focus activates.",
    ...
    func perform() async throws -> some IntentResult {
        guard let profile,
            ...
            DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.xera.xmenubar.focusFilterDeactivated"),
                object: nil,
        ...
        DistributedNotificationCenter.default().postNotificationName(
            Notification.Name("com.xera.xmenubar.focusFilterActivated"),
            object: nil,
