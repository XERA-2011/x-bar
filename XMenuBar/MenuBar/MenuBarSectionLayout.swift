...
nonisolated extension MenuBarSection {
    ...
    /// Whether notch overflow forces the XMenuBar Bar even though the display's own
    /// XMenuBar Bar setting is off.
    ///
    /// Split out as a pure function so the rule is testable without a live
    /// menu bar. Requires overflow to be enabled, the "use the XMenuBar Bar while
    ...
    static func forcesIceBarForNotchOverflow(
        ...
        useXMenuBarBarOnOverflow: Bool,
        hasEjectedItems: Bool
    ) -> Bool {
        overflowEnabled && useXMenuBarBarOnOverflow && hasEjectedItems
    }

    /// Whether the given section presents in the XMenuBar Bar.
    ///
    /// `displayUsesXMenuBarBar` sends every section there. `alwaysHiddenUsesXMenuBarBar`
    ...
    ///
    /// Notch overflow can force the XMenuBar Bar on top of this; see
    /// ``forcesIceBarForNotchOverflow(overflowEnabled:useXMenuBarBarOnOverflow:hasEjectedItems:)``.
    static func usesXMenuBarBar(
        for name: Name,
        displayUsesXMenuBarBar: Bool,
        alwaysHiddenUsesXMenuBarBar: Bool
    ) -> Bool {
        if displayUsesXMenuBarBar {
            return true
        }
        return name == .alwaysHidden && alwaysHiddenUsesXMenuBarBar
    }
