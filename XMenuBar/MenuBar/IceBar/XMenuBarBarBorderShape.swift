...
/// The XMenuBar Bar's rounded-rectangle geometry, serving as both its clip and
/// its border stroke.
///
...
nonisolated struct XMenuBarBarBorderShape: InsettableShape {
    ...
    /// The XMenuBar Bar's clip for a given content height: fully rounded ends get
    /// a circular half-height radius, square corners a continuous
    /// quarter-height one.
    static func xmenubarBarClip(height: CGFloat, hasRoundedShape: Bool) -> XMenuBarBarBorderShape {
    ...
    /// The border that traces ``xmenubarBarClip(height:hasRoundedShape:)``.
    ///
    ...
    static func xmenubarBarBorder(
        ...
        var shape = xmenubarBarClip(height: height, hasRoundedShape: hasRoundedShape)
        shape.omitTopEdge = !hasRoundedShape
        return shape.inset(by: borderWidth / 2)
    }

    func inset(by amount: CGFloat) -> XMenuBarBarBorderShape {
        var shape = self
