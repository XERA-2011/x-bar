...
nonisolated struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    var isSystemItem: Bool {
        switch namespace {
        case .controlCenter, .systemUIServer, .textInputMenuAgent, .weather, .passwords, .screenCaptureUI, .ssMenuAgent, .xmenubar, .gamePolicyAgent:
            return true
    ...
    ///
    /// User-created spacers (`XMenuBar.Spacer.<uuid>`) are deliberately NOT
    ...
    var isControlItem: Bool {
        if namespace == .xmenubar && title.hasPrefix(MenuBarSpacerManager.autosavePrefix) {
            return false
        }
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .xmenubar, title: identifier.rawValue, instanceIndex: 0)
    }
}
...
nonisolated extension MenuBarItemTag.Namespace {
    /// The namespace for the "XMenuBar" process.
    static let xmenubar = string(Constants.bundleIdentifier)
