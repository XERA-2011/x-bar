...
extension MenuBarItemManager {
    ...
    func relocateNewLeftmostItems(
        ...
        if isInStartupSettling {
            ...
            // The XMenuBar icon is exempt from the deferral above. macOS can
            ...
            // leaves the menu bar with no XMenuBar icon for as long as settling
            ...
            if let xmenubarIcon = LayoutSolver.planXMenuBarIconMove(
                items: items,
                hiddenBounds: bestBounds(for: controlItems.hidden)
            ) {
                return await relocateXMenuBarIcon(
                    xmenubarIcon,
        ...
        switch decision {
        case let .xmenubarIcon(xmenubarIcon):
            return await relocateXMenuBarIcon(
                xmenubarIcon,
                controlItems: controlItems,
                shouldBeginMove: shouldBeginMove
        ...
        case let .newHideableItem(candidate, identifierToMark):
            ...
            // XMenuBar's own spacers are placed by AppKit's autosave (seeded next
            // to the XMenuBar icon) — relocating them like new third-party items
                ...
                || appState?.spacerManager.ownsWindowID(candidate.windowID) == true
            {
                MenuBarItemManager.diagLog.info(
                    "Skipping new-item relocation for XMenuBar spacer \(candidate.logString)"
                )
    ...
    /// Moves the XMenuBar icon back to the right of the hidden divider, where it
    ...
    private func relocateXMenuBarIcon(
        _ xmenubarIcon: MenuBarItem,
        controlItems: ControlItemPair,
        shouldBeginMove: (@MainActor () -> Bool)? = nil
    ) async -> CacheDrivenMoveOutcome {
        let beginMove = shouldBeginMove
        ...
        if !LayoutSolver.isOnScreen(bounds: bestBounds(for: controlItems.hidden), screenFrames: screenFrames) {
            MenuBarItemManager.diagLog.warning(
                "Skipping XMenuBar icon relocation, the hidden divider is parked offscreen (minX=\(controlItems.hidden.bounds.minX)); moving the icon beside it would strand both"
            )
            return .noAttempt
        }
        MenuBarItemManager.diagLog.info("Relocating XMenuBar icon \(xmenubarIcon.logString) to visible section")
        let attemptRecorder = MoveAttemptAcceptanceRecorder()
        do {
            try await move(
                item: xmenubarIcon,
            ...
            MenuBarItemManager.diagLog.debug(
                "Skipping stale XMenuBar-icon relocation for \(xmenubarIcon.logString)"
            )
            return attemptRecorder.didAcceptMoveAttempt ? .failedAttempt : .noAttempt
        } catch {
            MenuBarItemManager.diagLog.error("Failed to relocate XMenuBar icon \(xmenubarIcon.logString): \(error)")
            await reportAutomaticMoveFailure(
                of: xmenubarIcon,
                to: .rightOfItem(controlItems.hidden),
                expectedSection: .visible,
    ...
    func recoverMisplacedVisibleControlItem(
        ...
        guard let misplaced = LayoutSolver.planXMenuBarIconMove(
            items: items,
            hiddenBounds: bestBounds(for: controlItems.hidden)
        ) else { return }
        _ = await relocateXMenuBarIcon(misplaced, controlItems: controlItems)
    }
