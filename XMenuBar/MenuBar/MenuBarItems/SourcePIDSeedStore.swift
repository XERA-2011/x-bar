...
/// Horizontal position is deliberately excluded because XMenuBar moves items and
/// recreating its dividers can shift the whole bar. The menu-bar lane, size,
/// title, owner name, and layer are expected to survive a XMenuBar-only relaunch.
...
///
/// On macOS 26, Control Center owns hosted item windows and XMenuBar resolves the
/// process behind each one through Accessibility. An item window can outlive a
/// XMenuBar process, so a confirmed attribution can bridge the next launch while
