...
/// XMenuBar's own concealment, the two `NSStatusItem Visible*` defaults, and the
...
final class MenuBarSpacerManager {
    /// Deliberately NOT under `XMenuBar.ControlItem.` — that prefix marks XMenuBar's
    ...
    static nonisolated let autosavePrefix = "XMenuBar.Spacer."

    /// Whether a cached item tag belongs to one of XMenuBar's user-created
    ...
    static nonisolated func isSpacerTag(_ tag: MenuBarItemTag) -> Bool {
        tag.namespace == .xmenubar && tag.title.hasPrefix(autosavePrefix)
    }
    ...
    private func reconcileStatusItems() {
        ...
        for spacer in spacers {
            ...
            // Seed new spacers just left of the XMenuBar icon — inside the visible
            ...
            if ControlItemDefaults[.preferredPosition, name] == nil {
                let xmenubarIconPosition: CGFloat =
                    ControlItemDefaults[.preferredPosition, ControlItem.Identifier.visible.rawValue] ?? 0
                ControlItemDefaults[.preferredPosition, name] = max(xmenubarIconPosition - 1, 0)
            }
