...
final class MenuBarSection {
    ...
    private var useIceBar: Bool {
        ...
        if Self.usesXMenuBarBar(
            for: name,
            displayUsesXMenuBarBar: displaySettings.useIceBar(for: displayID),
            alwaysHiddenUsesXMenuBarBar: displaySettings.useXMenuBarBarForAlwaysHidden(for: displayID)
        ) {
            return true
        }
    ...
    private static func forcesIceBarForNotchOverflow(
        ...
        forcesIceBarForNotchOverflow(
            ...
            useXMenuBarBarOnOverflow: settings.useXMenuBarBarOnNotchOverflow,
    ...
    func updateControlItemState(for screen: NSScreen? = nil) {
        ...
        let displaySettings = appState.settings.displaySettings
        let useIceBar = Self.usesXMenuBarBar(
            for: name,
            displayUsesXMenuBarBar: displaySettings.useIceBar(for: activeScreen.displayID),
            alwaysHiddenUsesXMenuBarBar: displaySettings.useXMenuBarBarForAlwaysHidden(for: activeScreen.displayID)
        )
    ...
    private func startRehideChecks() {
        ...
        case .smart, .timed:
            ...
            // that is still over the bar or the XMenuBar Bar is the #924 bug, and
