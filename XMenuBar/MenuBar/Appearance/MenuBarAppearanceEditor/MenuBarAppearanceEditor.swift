...
struct MenuBarAppearanceEditor: View {
    ...
    private var mainForm: some View {
        IceForm {
            ...
            XMenuBarBarAppearanceEditor(configuration: $appearanceManager.editedConfiguration)
...
private struct UnlabeledShapeEditor: View {
    ...
    /// the menu bar side of the border row is disabled. The XMenuBar Bar draws its
    /// border around its own panel and is unaffected.
    let shapeKind: MenuBarShapeKind

    /// Whether the XMenuBar Bar has been given its own appearance.
    ///
    ...
    let isXMenuBarBarOverridden: Bool
    ...
    private var borderToggle: some View {
        LabeledContent("Border") {
            HStack {
                ...
                Toggle("\(Constants.displayName) Bar", isOn: $configuration.borderOnXMenuBarBar)
                    .disabled(isXMenuBarBarOverridden)
            }
    ...
    private var partialEditor: some View {
        ...
        case .light:
            UnlabeledShapeEditor(
                ...
                isXMenuBarBarOverridden: configuration.xmenubarBarAppearance.overridesMenuBar
            )
        case .dark:
            UnlabeledShapeEditor(
...
// MARK: - XMenuBar Bar Editor

/// The section that forks the XMenuBar Bar's appearance away from the menu bar's.
///
...
private struct XMenuBarBarAppearanceEditor: View {
    @Binding var configuration: MenuBarAppearanceConfigurationV2

    private var appearance: XMenuBarBarAppearance {
        configuration.xmenubarBarAppearance
    }
    ...
    private var isEnabled: Binding<Bool> {
        Binding(
            get: { configuration.xmenubarBarAppearance.overridesMenuBar },
            set: { isOn in
                if isOn {
                    configuration.xmenubarBarAppearance = XMenuBarBarAppearance(
                        seededFrom: configuration.resolvedXMenuBarBarAppearance
                    )
                } else {
                    configuration.xmenubarBarAppearance.overridesMenuBar = false
                }
            }
        )
    }
    ...
    private var roundedShapeToggle: some View {
        Toggle("Rounded corners", isOn: $configuration.xmenubarBarAppearance.hasRoundedShape)
    }

    private var tintPicker: some View {
        LabeledContent("Tint") {
            HStack {
                IcePicker("Tint", selection: $configuration.xmenubarBarAppearance.tintKind) {
                    ForEach(XMenuBarBarAppearance.supportedTintKinds) { tintKind in
                        Text(tintKind.localized).tag(tintKind)
                    }
                }
                .labelsHidden()
                ...
                case .solid:
                    ColorPicker(
                        ...
                        selection: $configuration.xmenubarBarAppearance.tintColor,
                        supportsOpacity: false
                    )
                    .labelsHidden()
                case .gradient:
                    IceGradientPicker(
                        appearance.tintKind.localized,
                        gradient: $configuration.xmenubarBarAppearance.tintGradient,
                        supportsOpacity: false
                    )
                    .labelsHidden()
                default:
    ...
    private var tintOpacity: some View {
        if appearance.tintKind != .noTint {
            LabeledContent("Opacity") {
                IceSlider(
                    value: $configuration.xmenubarBarAppearance.tintOpacity,
                    in: 0 ... 1,
                    step: 0.05,
                    showsValue: false
                ) {
    ...
    private var borderToggle: some View {
        Toggle("Border", isOn: $configuration.xmenubarBarAppearance.hasBorder)
    }

    private var borderColor: some View {
        ColorPicker(
            "Border Color",
            selection: $configuration.xmenubarBarAppearance.borderColor,
            supportsOpacity: true
        )
    }
    ...
    private var borderWidth: some View {
        IcePicker(
            "Border Width",
            selection: $configuration.xmenubarBarAppearance.borderWidth
        ) {
            Text(verbatim: "1").tag(1.0)
            Text(verbatim: "2").tag(2.0)
            Text(verbatim: "3").tag(3.0)
