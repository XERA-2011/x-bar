...
nonisolated struct MenuBarAppearanceConfigurationV2: Hashable {
    ...
    var isDynamic: Bool
    /// The XMenuBar Bar's own appearance, honoured only when it opts in.
    var xmenubarBarAppearance: XMenuBarBarAppearance
    ...
    /// The values the XMenuBar Bar draws with, from whichever side owns them.
    ///
    ...
    @MainActor
    var resolvedXMenuBarBarAppearance: ResolvedXMenuBarBarAppearance {
        let partial = current
        guard xmenubarBarAppearance.overridesMenuBar else {
            return ResolvedXMenuBarBarAppearance(
                hasRoundedShape: hasRoundedShape,
                tintKind: partial.tintKind,
                tintColor: partial.tintColor,
                tintGradient: partial.tintGradient,
                // Not `partial.tintOpacity`. The XMenuBar Bar has always drawn the
                // inherited tint at a fixed opacity, so reading the menu bar's
                // here would restyle every existing install.
                tintOpacity: XMenuBarBarAppearance.inheritedTintOpacity,
                hasBorder: partial.borderOnXMenuBarBar,
                borderColor: partial.borderColor,
                borderWidth: partial.borderWidth
            )
        }
        return ResolvedXMenuBarBarAppearance(
            hasRoundedShape: xmenubarBarAppearance.hasRoundedShape,
            tintKind: xmenubarBarAppearance.tintKind,
            tintColor: xmenubarBarAppearance.tintColor,
            tintGradient: xmenubarBarAppearance.tintGradient,
            tintOpacity: xmenubarBarAppearance.tintOpacity,
            hasBorder: xmenubarBarAppearance.hasBorder,
            borderColor: xmenubarBarAppearance.borderColor,
            borderWidth: xmenubarBarAppearance.borderWidth
        )
    }
}
...
nonisolated extension MenuBarAppearanceConfigurationV2 {
    static let defaultConfiguration = MenuBarAppearanceConfigurationV2(
        ...
        notchMargin: 0,
        isDynamic: false,
        xmenubarBarAppearance: .defaultConfiguration
    )
}
...
nonisolated extension MenuBarAppearanceConfigurationV2: Codable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        try self.init(
            ...
            xmenubarBarAppearance: container.decodeIfPresent(XMenuBarBarAppearance.self, forKey: .xmenubarBarAppearance) ?? Self.defaultConfiguration.xmenubarBarAppearance
        )
    }
    ...
    func encode(to encoder: any Encoder) throws {
        ...
        try container.encode(xmenubarBarAppearance, forKey: .xmenubarBarAppearance)
    }
}
...
nonisolated struct MenuBarAppearancePartialConfiguration: Hashable {
    ...
    /// Whether the shape border is drawn around the XMenuBar Bar.
    ///
    /// The XMenuBar Bar is the panel in `IceBar.swift`, which still carries the
    ...
    var hasBorder: Bool {
        get {
            borderOnMenuBar || borderOnXMenuBarBar
        }
        set {
            borderOnMenuBar = newValue
            borderOnXMenuBarBar = newValue
        }
    }
}
...
nonisolated extension MenuBarAppearancePartialConfiguration: Codable {
    private enum CodingKeys: CodingKey {
        case hasShadow
        /// The single border flag that predates the menu bar / XMenuBar Bar split.
        ///
    ...
    init(from decoder: Decoder) throws {
        ...
        // to the menu bar and the XMenuBar Bar at once, so it seeds both flags.
        ...
        try self.init(
            ...
            borderOnXMenuBarBar: container.decodeIfPresent(Bool.self, forKey: .borderOnXMenuBarBar) ?? legacyHasBorder ?? Self.defaultConfiguration.borderOnXMenuBarBar,
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(borderOnXMenuBarBar, forKey: .borderOnXMenuBarBar)
...
// MARK: - XMenuBarBarAppearance

/// The XMenuBar Bar's own appearance, drawn in place of the menu bar's when
/// ``overridesMenuBar`` is set.
///
/// The XMenuBar Bar has always borrowed the menu bar's shape, tint and border so
...
nonisolated struct XMenuBarBarAppearance: Hashable {
    /// Whether the XMenuBar Bar draws with these values instead of the menu bar's.
...
// MARK: Default XMenuBarBarAppearance

nonisolated extension XMenuBarBarAppearance {
    /// The opacity the XMenuBar Bar has always drawn its inherited tint at.
    ///
    ...
    static let defaultConfiguration = XMenuBarBarAppearance(
...
nonisolated extension XMenuBarBarAppearance {
    /// The tint kinds the XMenuBar Bar can draw.
    ///
    ...
    /// An override seeded from what the XMenuBar Bar is drawing right now.
    ///
    ...
    init(seededFrom resolved: ResolvedXMenuBarBarAppearance) {
        self.init(
...
// MARK: XMenuBarBarAppearance: Codable

nonisolated extension XMenuBarBarAppearance: Codable {
    private enum CodingKeys: CodingKey {
...
// MARK: - ResolvedXMenuBarBarAppearance

/// The values the XMenuBar Bar actually draws with, after the override has been
...
nonisolated struct ResolvedXMenuBarBarAppearance: Hashable {
    var hasRoundedShape: Bool
    var tintKind: MenuBarTintKind
    var tintColor: CGColor
    var tintGradient: IceGradient
