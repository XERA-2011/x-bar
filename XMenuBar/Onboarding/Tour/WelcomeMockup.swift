...
/// kind of status items XMenuBar actually manages. Split across three rings so
...
/// Drives the welcome scene: the XMenuBar icon appears, menu bar glyphs orbit
/// out around it like little planets, then collapse back in and hide behind
/// the icon — a small preview of what XMenuBar actually does to the real menu
/// bar. Clicking the icon toggles the same show/hide at any time.
@MainActor
@Observable
final class XMenuBarWelcomeModel {
...
/// that state.
struct XMenuBarWelcomeMockup: View {
    let model: XMenuBarWelcomeModel
