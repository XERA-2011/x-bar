...
@MainActor
@Observable
final class XMenuBarAppearanceMockupModel {
...
struct AppearanceSlideMockup: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            SlideHUD {
                HStack(spacing: 0) {
                    ForEach(Array(XMenuBarAppearanceMockupModel.styleLabels.enumerated()), id: \.offset) { i, label in
                        Button(label) {
                            onInteraction()
                            model.select(i)
                        }
...
@MainActor
@Observable
final class XMenuBarHotkeysMockupModel {
...
@MainActor
@Observable
final class XMenuBarProfilesMockupModel {
    struct FocusMode {
        let name: String
        let symbol: String
        let items: [String]
...
struct ProfilesSlideMockup: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            SlideHUD {
                HStack(spacing: 0) {
                    ForEach(Array(XMenuBarProfilesMockupModel.focusModes.enumerated()), id: \.offset) { i, mode in
                        Button {
                            onInteraction()
                            model.select(i)
                        } label: {
