...
struct IceSettingsImporter {
    ...
    /// Source preferences are injectable so conversion can be verified
    /// without mutating real Ice or XMenuBar settings.
    ...
    private func importGeneralSettings(from iceSettings: [String: Any]) -> Int {
        ...
        let mappings: [(Defaults.Key, String)] = [
            ...
            // Legacy XMenuBar Bar keys kept for migration compatibility
            (.useIceBar, "UseIceBar"),
        ...
        // Generate per-display configurations when importing XMenuBar Bar settings
        imported += importPerDisplayIceBarSettings(from: iceSettings)

        return imported
    }

    /// Generates per-display XMenuBar Bar configurations from imported Ice settings.
    private func importPerDisplayIceBarSettings(from iceSettings: [String: Any]) -> Int {
        ...
        do {
            ...
            diagLog.info("Generated per-display XMenuBar Bar configs for \(configs.count) display(s) from Ice import")
            return 1
        } catch {
            diagLog.error("Failed to encode per-display XMenuBar Bar configs during import: \(error)")
            return 0
        }
    }
