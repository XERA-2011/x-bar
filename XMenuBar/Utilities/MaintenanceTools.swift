...
///
/// These operate on the current user's Library and TCC entries for XMenuBar's own
...
nonisolated enum MaintenanceTools {
    private static let legacyCacheDirectoryName = "com.xera.xmenubar"
    ...
    /// Deletes XMenuBar's user cache directory (`~/Library/Caches/<bundle id>`).
    ...
    /// Resets Accessibility and Screen Recording TCC decisions for XMenuBar so the
