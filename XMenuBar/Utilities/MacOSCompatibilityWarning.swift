    ...
    static nonisolated func prompt(
        ...
        guard canSubscribe, UpdateChannel.alpha.isAvailable(on: version) else {
            return Prompt(
                ...
                message: String(
                    localized: """
                    This version of XMenuBar is not yet compatible with macOS \(release). Preview builds are available on GitHub Releases, and support will be delivered through the alpha update channel.
                    """
                ),
        ...
        return Prompt(
            ...
            message: String(
                localized: """
                This version of XMenuBar is not yet compatible with macOS \(release). Support arrives through the alpha channel, which carries the rewritten app. XMenuBar can subscribe you and check for a build now. If none has been published yet, it opens the preview builds on GitHub.
                """
            ),
