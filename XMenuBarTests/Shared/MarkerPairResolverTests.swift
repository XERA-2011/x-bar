...
struct MarkerPairResolverTests {
    ...
    private let xmenubarBundleID = "com.xera.xmenubar"
    private let ccBundleID = "com.apple.controlcenter"
    private let xmenubarControlItemPrefix = "XMenuBar.ControlItem."
    ...
    func agentSceneResolvesViaTitleLookup() {
        ...
        let result = MarkerPairResolver.resolve(
            ...
            markers: markers,
            xmenubarBundleID: xmenubarBundleID,
    ...
    /// Marker's CG owner is the widget's real app (not CC, not XMenuBar):
    ...
    /// Marker's CG owner resolves to XMenuBar itself: rejected, falls
    /// through to title lookup. The title-lookup result must also
    /// be checked for XMenuBar self-attribution (see the next test).
    @Test("A XMenuBar owner falls through to the title lookup")
    func xmenubarOwnerFallsThroughToTitleLookup() {
        let icons = [icon(windowID: 1, title: "Item-0")]
    ...
    /// Both paths resolve to XMenuBar: no resolution emitted. Defensive
    /// guarantee that XMenuBar's own PID is never attributed to a
    /// third-party widget regardless of where the lookup happens to
    /// land.
    @Test("Both paths resolving to XMenuBar produces no result")
    func bothPathsResolveToXMenuBarProducesNoResult() {
        let icons = [icon(windowID: 1, title: "Item-0")]
        let markers = [marker(windowID: 2, title: "com.example.widget", owningPID: 100)]
        let result = MarkerPairResolver.resolve(
            unresolvedIcons: icons,
            ...
            xmenubarBundleID: xmenubarBundleID,
            ccBundleID: ccBundleID,
            pidToBundleID: { _ in self.xmenubarBundleID },
    ...
    func extractMarkersExcludesGenericTitles() {
        ...
        let markers = MarkerPairResolver.extractMarkers(
            from: windows,
            xmenubarControlItemPrefix: xmenubarControlItemPrefix,
            xmenubarBundleID: xmenubarBundleID
        )
        #expect(markers.map(\.windowID) == [4])
    }

    /// XMenuBar control items are excluded by the XMenuBar.ControlItem.
    /// prefix even though their titles contain dots.
    @Test("extractMarkers excludes XMenuBar control items")
    func extractMarkersExcludesXMenuBarControlItems() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "XMenuBar.ControlItem.Hidden", CGSize(width: 5016, height: 33), nil),
            (2, "XMenuBar.ControlItem.AlwaysHidden", CGSize(width: 5016, height: 33), nil),
    ...
    /// The XMenuBar self-registration window (title equals the XMenuBar bundle
    /// identifier) is excluded so XMenuBar's own PID can never be
    /// attributed to a third-party widget via the title-lookup path.
    @Test("extractMarkers excludes the XMenuBar self-registration window")
    func extractMarkersExcludesXMenuBarSelfRegistration() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "com.xera.xmenubar", CGSize(width: 33, height: 33), nil), // XMenuBar self
