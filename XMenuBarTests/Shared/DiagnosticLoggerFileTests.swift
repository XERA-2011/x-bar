...
/// a user's `~/Library/Logs/XMenuBar` directory from growing without bound.
///
...
/// file in the developer's own `~/Library/Logs/XMenuBar`. `logDirectory`,
...
struct DiagnosticLoggerFileTests {
    ...
    func attachingCreatesTheDirectoryTree() async throws {
        try await withTemporaryLogDirectory { tmp in
            let file = tmp
                ...
                .appendingPathComponent("xmenubar.log")
    ...
    func attachingWritesAHeader() async throws {
        try await withTemporaryLogDirectory { tmp in
            let file = tmp.appendingPathComponent("xmenubar.log")
            ...
            #expect(text.contains("XMenuBar Diagnostic Log"))
    ...
    func reattachingToTheSameFileIsANoOp() async throws {
        try await withTemporaryLogDirectory { tmp in
            ...
            let text = contents(of: file)
            #expect(occurrences(of: "XMenuBar Diagnostic Log", in: text) == 1)
    ...
    func attachingToAnotherFileSwitchesOver() async throws {
        try await withTemporaryLogDirectory { tmp in
            ...
            #expect(contents(of: second).contains("XMenuBar Diagnostic Log"))
    ...
    func attachingIntoAnUnwritableDirectoryFails() async throws {
        try await withTemporaryLogDirectory { tmp in
            ...
            DiagnosticLogger.shared.attachToFile(at: locked.appendingPathComponent("xmenubar.log"))
    ...
    @Test("The log directory is Library/Logs/XMenuBar inside the user's home")
    func logDirectoryIsUnderTheUserLibrary() {
        let directory = DiagnosticLogger.shared.logDirectory

        #expect(directory.lastPathComponent == "XMenuBar")
