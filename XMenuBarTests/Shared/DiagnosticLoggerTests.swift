...
struct DiagnosticLoggerRetentionTests {
    private let directory = URL(fileURLWithPath: "/tmp/XMenuBarLogsTests", isDirectory: true)
    ...
    func prunesFilesPastRetentionWindow() {
        let now = Date()
        let fresh = logFile("xmenubar_fresh.log")
        let old = logFile("xmenubar_old.log")
    ...
    func neverPrunesProtectedFiles() {
        let now = Date()
        let current = logFile("xmenubar_current.log")
        let previous = logFile("xmenubar_previous.log")
    ...
    func enforcesCountCap() {
        let now = Date()
        let newest = logFile("xmenubar_1.log")
        let middle = logFile("xmenubar_2.log")
        let oldest = logFile("xmenubar_3.log")
    ...
    func keepsFilesInsideBothLimits() {
        ...
        let stale = DiagnosticLogger.filesToPrune(
            [(url: logFile("xmenubar_a.log"), created: daysAgo(0.1, from: now)),
             (url: logFile("xmenubar_b.log"), created: daysAgo(0.2, from: now))],
            retentionDays: 2,
            maxCount: 100,
            now: now,
            protected: []
    ...
    func prunesEverythingWhenProtectedFillsTheCap() {
        let now = Date()
        let current = logFile("xmenubar_current.log")
        let spare = logFile("xmenubar_spare.log")
    ...
    func breaksAgeTiesByPath() {
        ...
        let first = logFile("xmenubar_a.log")
        let second = logFile("xmenubar_b.log")

        // Only one of the two fits under the cap. Directory order must not
        // decide which; the tie is broken by path, so `xmenubar_a.log` survives.
        let stale = DiagnosticLogger.filesToPrune(
...
struct DiagnosticLoggerRotationPolicyTests {
    ...
    func clampsExtremeFileCount() {
        ...
        let stale = DiagnosticLogger.filesToPrune(
            ...
            now: Date(),
            protected: [URL(fileURLWithPath: "/tmp/XMenuBarLogsTests/xmenubar.log")]
        )
        #expect(stale.isEmpty)
    }
...
struct DiagnosticLoggerFileNameTests {
    private let directory = URL(fileURLWithPath: "/tmp/XMenuBarLogsTests", isDirectory: true)
    private let baseName = "xmenubar_2026-06-07_09-48-52"
    ...
    func usesBaseNameWhenFree() {
        ...
        #expect(url.lastPathComponent == "xmenubar_2026-06-07_09-48-52.log")
    }
    ...
    func appendsSuffixOnCollision() {
        ...
        #expect(url.lastPathComponent == "xmenubar_2026-06-07_09-48-52_2.log")
    }
    ...
    func incrementsSuffixUntilFree() {
        ...
        #expect(url.lastPathComponent == "xmenubar_2026-06-07_09-48-52_3.log")
    }
}
