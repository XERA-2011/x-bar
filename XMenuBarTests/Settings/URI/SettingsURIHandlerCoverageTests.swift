...
///   arm — identical in effect, and the one a `xmenubar://toggle` URL uses to flip
...
/// so the suite never touches the real `com.xera.xmenubar` domain and always
...
struct SettingsURIHandlerCoverageTests {
    ...
    struct TogglingANamedDisplay {
        ...
        @Test(
            ...
            arguments: ["useIceBar", "alwaysShowHiddenItems", "useXMenuBarBarForAlwaysHidden"]
        )
    ...
    struct UnnamedEnumerationValue {
        ...
        /// that stored state is from outside XMenuBar.
    ...
    struct ParserEdges {
        /// A `xmenubar://` URL is assembled by whoever sends it, and a stray space
