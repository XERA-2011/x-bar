...
/// functions run on behalf of a *third-party app* that sent a `xmenubar://` URL,
...
/// land in a throwaway store rather than the real `com.xera.xmenubar` domain,
...
struct SettingsURIHandlerApplyTests {
    ...
    @Test("alwaysShowHiddenItems defaults to the non-XMenuBar-Bar displays")
    ...
    @Test("An invalid per-display boolean is refused", arguments: ["useIceBar", "alwaysShowHiddenItems", "useXMenuBarBarForAlwaysHidden"])
