...
//
// Source: github.com/xmenubar-app/raycast-extension
//   - src/data/index.ts — DIRECT_ACTIONS (6) and SETTINGS_ACTIONS (4)
//   - src/utils/openUrl.ts — buildXMenuBarUrl() composes `xmenubar://<action>?<query>`
//
// buildXMenuBarUrl drops falsy query values, so a parameter is either present with
// a non-empty value or absent entirely. Keys are compile-time constants in the
// action table, so a bare `xmenubar://toggle` is unreachable from Raycast.

/// Every URL the Raycast extension is able to emit.
private let raycastURIs: [String] = [
    "xmenubar://toggle-hidden",
    "xmenubar://toggle-always-hidden",
    "xmenubar://search",
    "xmenubar://toggle-xmenubarbar",
    "xmenubar://toggle-application-menus",
    "xmenubar://open-settings",
    "xmenubar://authorize",
    "xmenubar://toggle?key=autoRehide",
    "xmenubar://toggle?key=showOnHover",
    "xmenubar://toggle?key=hideApplicationMenus",
]
...
//   "xmenubar://get?key=all&callback="   (callback value interpolated)
//   "xmenubar://toggle?key="             (key value interpolated)
//   "droppy://xmenubar-response"         (its callback scheme)
//
// Droppy emits no `xmenubar://set` at all. Its onboarding is a text instruction
// telling the user to enable the Settings URI scheme and approve Droppy when
// XMenuBar prompts — it does not probe with a parameterless URL to trigger the
// dialog.

/// Every URL shape Droppy is able to emit.
private let droppyURIs: [String] = [
    "xmenubar://get?key=all&callback=droppy://xmenubar-response",
    "xmenubar://get?key=all&callback=droppy%3A%2F%2Fxmenubar-response",
    "xmenubar://get?key=all&callback=droppy://xmenubar-response&requestId=7",
    "xmenubar://get?key=all&callback=droppy://xmenubar-response&broadcast=true",
    "xmenubar://get?key=all&callback=droppy://xmenubar-response&requestId=7&broadcast=true",
    "xmenubar://toggle?key=autoRehide",
    "xmenubar://toggle?key=useIceBar",
]
...
struct IntegrationURIContractTests {
    ...
    struct Raycast {
        @Test("Direct actions resolve to their action", arguments: [
            ("xmenubar://toggle-hidden", SettingsURIAction.toggleHidden),
            ("xmenubar://toggle-always-hidden", .toggleAlwaysHidden),
            ("xmenubar://search", .search),
            ("xmenubar://toggle-xmenubarbar", .toggleXMenuBarbar),
            ("xmenubar://toggle-application-menus", .toggleApplicationMenus),
            ("xmenubar://open-settings", .openSettings),
        ])
        func directActions(uri: String, action: SettingsURIAction) throws {
            #expect(try parse(uri).route == .action(action))
        }

        /// Raycast's onboarding depends on this: it sends `xmenubar://authorize`
        /// rather than probing with an incomplete settings URL.
        @Test("Authorize command reaches the authorization gate")
        func authorize() throws {
            let request = try parse("xmenubar://authorize")
            #expect(request.route == .authorize)
            #expect(request.requiresAuthorization)
        }

        @Test("Settings toggles carry their key", arguments: [
            ("xmenubar://toggle?key=autoRehide", "autoRehide"),
            ("xmenubar://toggle?key=showOnHover", "showOnHover"),
            ("xmenubar://toggle?key=hideApplicationMenus", "hideApplicationMenus"),
        ])
        func settingsToggles(uri: String, key: String) throws {
            #expect(try parse(uri).route == .toggle(key: key, displayUUID: nil))
        }

        /// `buildXMenuBarUrl` forwards any query pair it is handed, so unknown
        ...
        func unknownParametersIgnored() throws {
            #expect(try parse("xmenubar://toggle?key=showOnHover&label=Show+on+hover+%26+delay").route
    ...
    struct Droppy {
        ...
        func snapshotQuery(uri: String) throws {
            ...
            #expect(key == "all")
            #expect(callback == "droppy://xmenubar-response")
        }
        ...
        /// Droppy interpolates the key into `xmenubar://toggle?key=`. An empty
        ...
        func emptyInterpolatedKey() throws {
            #expect(try parse("xmenubar://toggle?key=").route == .toggle(key: "", displayUUID: nil))
        }
