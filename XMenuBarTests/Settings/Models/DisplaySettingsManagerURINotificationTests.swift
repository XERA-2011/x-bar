...
struct DisplaySettingsManagerURINotificationTests {
    ...
    // MARK: useXMenuBarBarForAlwaysHidden

    @Test("A useXMenuBarBarForAlwaysHidden value is written to the named display", arguments: [true, false])
    func useXMenuBarBarForAlwaysHiddenValueIsApplied(value: Bool) throws {
        try withScratchDefaults { _ in
            let manager = makeManager(.defaultConfiguration.withUseXMenuBarBarForAlwaysHidden(!value))

            manager.handleExternalPerDisplaySettingsChange(perDisplayChange([
                "key": "useXMenuBarBarForAlwaysHidden",
                "scope": "specific:\(offscreenUUID)",
                "value": value,
            ]))

            #expect(manager.configurations[offscreenUUID]?.useXMenuBarBarForAlwaysHidden == value)
        }
    }

    @Test("A useXMenuBarBarForAlwaysHidden toggle flips the named display", arguments: [true, false])
    func useXMenuBarBarForAlwaysHiddenToggleFlipsTheDisplay(initial: Bool) throws {
        try withScratchDefaults { _ in
            let manager = makeManager(.defaultConfiguration.withUseXMenuBarBarForAlwaysHidden(initial))
            ...
            #expect(manager.configurations[offscreenUUID]?.useXMenuBarBarForAlwaysHidden == !initial)
        }
    }

    @Test("A useXMenuBarBarForAlwaysHidden notification with no usable payload changes nothing")
    func useXMenuBarBarForAlwaysHiddenWithoutAUsablePayloadIsIgnored() throws {
        try withScratchDefaults { _ in
            let manager = makeManager()
            let before = manager.configurations
    ...
    /// The `allNonIceBar` broadcast of a `useXMenuBarBarForAlwaysHidden` value,
    /// which walks the same non-XMenuBar-Bar filter as `alwaysShowHiddenItems`.
    /// Vacuously true with no displays attached; the assertion about
    /// ``offscreenUUID`` holds either way.
    @Test("An allNonIceBar useXMenuBarBarForAlwaysHidden value reaches attached displays but not a stored one")
    func allNonIceBarUseXMenuBarBarForAlwaysHiddenValueReachesAttachedDisplays() throws {
        try withScratchDefaults { _ in
            let manager = makeManager()
            ...
            for display in manager.connectedDisplays() {
                #expect(manager.configuration(forUUID: display.id).useXMenuBarBarForAlwaysHidden, "\(display.id)")
            }
            #expect(
                manager.configurations[offscreenUUID]?.useXMenuBarBarForAlwaysHidden == false,
                "a stored but unattached display must not be swept up"
            )
        }
    }
    ...
    @Test("An allNonIceBar useXMenuBarBarForAlwaysHidden toggle flips attached displays but not a stored one")
    func allNonIceBarUseXMenuBarBarForAlwaysHiddenToggleFlipsAttachedDisplays() throws {
        try withScratchDefaults { _ in
            let manager = makeManager()
    ...
    /// `useXMenuBarBarForAlwaysHidden` only implements the `allNonIceBar` scope;
    /// any other scope-wide request falls into the not-implemented arm and
    /// must change nothing, set or toggle alike.
    @Test("A useXMenuBarBarForAlwaysHidden broadcast on an unimplemented scope changes nothing", arguments: [
        "value",
        "toggle",
    ])
    func useXMenuBarBarForAlwaysHiddenUnimplementedScopeIsIgnored(payloadKey: String) throws {
        try withScratchDefaults { _ in
            let manager = makeManager()
            let before = manager.configurations
