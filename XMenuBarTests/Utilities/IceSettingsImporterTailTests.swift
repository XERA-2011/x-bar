...
/// validates it before it is copied into XMenuBar's own domain — so absent keys,
...
///   the tests, so only the deterministic "no XMenuBar Bar to convert" side is
///   covered here.
...
struct IceSettingsImporterTailTests {
    ...
    /// It matters here because XMenuBar's own `Defaults.Key` raw values are the
    ...
    private func makeSource(_ values: [String: Any]) throws -> (defaults: UserDefaults, domainName: String) {
        let domainName = "com.xera.xmenubarTests.IceSettingsImporterTail.\(UUID().uuidString)"
    ...
    /// type, so a key Ice stored as a string lands in XMenuBar's domain as a string
    ...
    @Test("A XMenuBar Bar that Ice had switched off generates no per-display configuration")
    ...
    func hotkeysDictionaryWithoutDataValuesImportsNothing() throws {
        try withScratchDefaults { suite in
            ...
            // Writing an empty dictionary would clear whatever hotkeys XMenuBar
    ...
    func v2AppearanceIsPassedThroughUnchanged() throws {
        try withScratchDefaults { _ in
            ...
            // Ice's V2 format is XMenuBar's V2 format, so the importer must not
