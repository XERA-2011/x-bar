...
///
/// The only surviving migration converts the global XMenuBar Bar switches into
...
struct MigrationTests {
    ...
    /// With the XMenuBar Bar switched off there is nothing to convert, but the
    /// latch still has to be set — otherwise the migration reconsiders the
    /// legacy keys at every launch.
    @Test("A disabled XMenuBar Bar writes no configurations but still latches")
    ...
    @Test("An enabled XMenuBar Bar writes a decodable configuration payload")
