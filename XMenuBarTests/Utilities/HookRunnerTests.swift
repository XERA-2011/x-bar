...
final class HookRunnerTests {
    ...
    func contextIsPassedAsEnvironment() async throws {
        let path = try writeScript(
            """
            #!/bin/sh
            echo "$XMENUBAR_HOOK_PHASE|$XMENUBAR_HOOK_SCOPE|$XMENUBAR_PROFILE_ID|$XMENUBAR_PROFILE_NAME|$XMENUBAR_PREVIOUS_PROFILE_ID|$XMENUBAR_PREVIOUS_PROFILE_NAME"
            """
        )
        let profileID = UUID()
        let previousID = UUID()
    ...
    func absentPreviousProfileIsEmpty() async throws {
        ...
        let path = try writeScript(
            """
            #!/bin/sh
            echo "[$XMENUBAR_PREVIOUS_PROFILE_ID][$XMENUBAR_PREVIOUS_PROFILE_NAME]"
            """
        )
