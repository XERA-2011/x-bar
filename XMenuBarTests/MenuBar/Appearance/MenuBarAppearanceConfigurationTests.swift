...
struct MenuBarAppearanceConfigurationTests {
    ...
    struct MenuBarAppearanceBorderTests {
        private func partialConfiguration(
            ...
            partial.borderOnXMenuBarBar = borderOnXMenuBarBar
            return partial
        }
        ...
        func hasBorderReflectsBothFlags(menuBar: Bool, xmenubarBar: Bool, expected: Bool) {
            let partial = partialConfiguration(borderOnMenuBar: menuBar, borderOnXMenuBarBar: xmenubarBar)
        ...
        func settingHasBorderWritesBothFlags(newValue: Bool) {
            ...
            #expect(partial.borderOnXMenuBarBar == newValue)
        }

        @Test("Both flags survive a round trip")
        func encodeDecode() throws {
            let original = partialConfiguration(borderOnMenuBar: false, borderOnXMenuBarBar: true)
            ...
            #expect(decoded.borderOnXMenuBarBar)
        }
        ...
        func decodeLegacyPayload(legacyHasBorder: Bool) throws {
            ...
            #expect(decoded.borderOnXMenuBarBar == legacyHasBorder)
        }
    ...
    /// `0.11.10` release — which reaches XMenuBar only through the Ice importer.
    ///
    ...
    struct MenuBarAppearanceV1MigrationTests {
        ...
        func migratedValuesApplyToEveryAppearance() {
            ...
            for partial in partials {
                ...
                #expect(partial.borderOnXMenuBarBar)
    ...
    /// The XMenuBar Bar borrowed the menu bar's shape, tint and border for its
    /// whole life before this, so the tests that matter here are the ones
    /// pinning that it still does until someone opts out.
    @MainActor
    @Suite("XMenuBar Bar appearance")
    struct XMenuBarBarAppearanceTests {
        @Test("The XMenuBar Bar follows the menu bar by default")
        func defaultDoesNotOverride() {
            #expect(!XMenuBarBarAppearance.defaultConfiguration.overridesMenuBar)
            #expect(!MenuBarAppearanceConfigurationV2.defaultConfiguration.xmenubarBarAppearance.overridesMenuBar)
        }
        ...
        func inheritsFromMenuBar() {
            let configuration = withMutableCopy(of: MenuBarAppearanceConfigurationV2.defaultConfiguration) { config in
                ...
                config.staticConfiguration.borderOnXMenuBarBar = true
                config.staticConfiguration.borderWidth = 3
            }

            let resolved = configuration.resolvedXMenuBarBarAppearance
        ...
        func inheritedTintOpacityIgnoresMenuBar() {
            ...
            #expect(configuration.resolvedXMenuBarBarAppearance.tintOpacity == XMenuBarBarAppearance.inheritedTintOpacity)
        }

        /// The border is the one property that was already forked, so the menu
        /// bar's own `borderOnMenuBar` must not leak into the panel.
        @Test("A menu-bar-only border does not reach the XMenuBar Bar")
        func menuBarOnlyBorderIsNotInherited() {
            let configuration = withMutableCopy(of: MenuBarAppearanceConfigurationV2.defaultConfiguration) { config in
                config.staticConfiguration.borderOnMenuBar = true
                config.staticConfiguration.borderOnXMenuBarBar = false
            }

            #expect(!configuration.resolvedXMenuBarBarAppearance.hasBorder)
        }
        ...
        func overrideWins() {
            let configuration = withMutableCopy(of: MenuBarAppearanceConfigurationV2.defaultConfiguration) { config in
                ...
                config.staticConfiguration.tintColor = .white
                config.staticConfiguration.borderOnXMenuBarBar = true
                config.xmenubarBarAppearance = XMenuBarBarAppearance(
        ...
        func seedingIsInert() {
            ...
            let before = configuration.resolvedXMenuBarBarAppearance

            let seeded = withMutableCopy(of: configuration) { config in
                config.xmenubarBarAppearance = XMenuBarBarAppearance(seededFrom: before)
            }

            #expect(seeded.resolvedXMenuBarBarAppearance == before)
        }

        /// The panel has never drawn the wallpaper-derived kinds, so seeding
        /// one would hand the editor a selection it cannot render.
        @Test("Seeding drops a tint kind the XMenuBar Bar cannot draw")
        func seedingDropsUnsupportedTintKind() {
            let configuration = withMutableCopy(of: MenuBarAppearanceConfigurationV2.defaultConfiguration) { config in
                config.staticConfiguration.tintKind = .adaptive
            }

            let seeded = XMenuBarBarAppearance(seededFrom: configuration.resolvedXMenuBarBarAppearance)
        ...
        func encodeDecode() throws {
            let original = XMenuBarBarAppearance(
                overridesMenuBar: true,
            ...
            let data = try JSONEncoder().encode(original)
            let decoded = try JSONDecoder().decode(XMenuBarBarAppearance.self, from: data)
        ...
        func decodeWithMissingField() throws {
            ...
            #expect(!decoded.xmenubarBarAppearance.overridesMenuBar)
        }
    }
}
