...
/// (1) XMenuBar icon, (2) non-hideable system item, (3) new hideable item,
...
struct PlanLeftmostMoveTests {
    ...
    /// The XMenuBar visible-control icon left of the divider triggers the
    /// XMenuBar-icon recovery branch.
    @Test("The XMenuBar icon left of the divider takes the XMenuBar-icon branch")
    func xmenubarIconLeftOfDividerTriggersXMenuBarIconBranch() {
        let xmenubar = leftmostItem(
        ...
        let decision = LayoutSolver.planLeftmostMove(
            ...
            observation: LayoutSolver.LeftmostObservation(
                ...
                sectionByWindowID: [xmenubar.windowID: .hidden],
                previousWindowIDs: []
            ),
            savedSectionOrder: [:],
        ...
        if case let .xmenubarIcon(item) = decision {
            #expect(item.windowID == 700)
        } else {
            Issue.record("expected .xmenubarIcon, got \(decision)")
        }
    }
    ...
    /// macOS hosts but XMenuBar cannot yet attribute, so a saved entry under it
    ...
    // MARK: - XMenuBar icon, standalone

    /// planXMenuBarIconMove is what the startup-settling path calls, before the
    /// other items' namespace tags are trustworthy. It must agree with the
    /// XMenuBar-icon branch of the full planner.
    @Test("planXMenuBarIconMove finds the XMenuBar icon left of the divider")
    func planXMenuBarIconMoveFindsIconLeftOfDivider() {
        let xmenubar = leftmostItem(tag: .visibleControlItem, x: 100, windowID: 700)

        let icon = LayoutSolver.planXMenuBarIconMove(items: [xmenubar], hiddenBounds: hiddenBounds)
    ...
    @Test("planXMenuBarIconMove returns nil when the XMenuBar icon is already placed")
    func planXMenuBarIconMoveIgnoresIconRightOfDivider() {
        let xmenubar = leftmostItem(tag: .visibleControlItem, x: 500, windowID: 700)
    ...
    /// The settling path must act on the XMenuBar icon only. Third-party items
    ...
    @Test("planXMenuBarIconMove ignores non-XMenuBar items left of the divider")
    func planXMenuBarIconMoveIgnoresOtherLeftmostItems() {
        ...
        let icon = LayoutSolver.planXMenuBarIconMove(
