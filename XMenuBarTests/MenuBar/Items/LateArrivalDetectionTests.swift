...
struct LateArrivalDetectionTests {
    ...
    /// The behaviour the detector exists for: an app launches after XMenuBar, its
    ...
    /// Control items are XMenuBar's own dividers, not profile members, and are
    ...
    func controlItemsAreNeverLateArrivals() {
        let divider = MenuBarItem.fixture(
            tag: MenuBarItemTag(
                namespace: .string("com.xera.xmenubar"),
                title: "XMenuBar.ControlItem.Hidden"
            ),
            windowID: 27481,
            sourcePID: 4321
        )
