//
//  XMenuBarBarBorderShapeTests.swift
//  Project: XMenuBar
//
...
/// Covers the geometry the XMenuBar Bar's clip and border share, and the path
...
@Suite("XMenuBar Bar border shape")
struct XMenuBarBarBorderShapeTests {
    ...
    func keepingTopEdgeStaysClosed() {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 15, cornerStyle: .circular, omitTopEdge: false)
                .path(in: rect)
        )
    ...
    func omittingTopEdgeRemovesIt(style: RoundedCornerStyle) {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 7.5, cornerStyle: style, omitTopEdge: true)
                .path(in: rect)
        )
        #expect(!outline.isClosed)
        #expect(!outline.hasTopEdge(of: rect))
    ...
    func openEndsAreTopCorners() {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 7.5, cornerStyle: .continuous, omitTopEdge: true)
                .path(in: rect)
        )
    ...
    func zeroRadiusDrawsThreeEdges() {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 0, omitTopEdge: true).path(in: rect)
        )
    ...
    func insetPastRadiusDegenerates() {
        let shape = XMenuBarBarBorderShape(cornerRadius: 4, omitTopEdge: true).inset(by: 6)
    ...
    func oversizedRadiusIsClamped() {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 500, cornerStyle: .circular, omitTopEdge: true)
                .path(in: rect)
        )
        #expect(outline.boundingRect.isNear(rect))
        #expect(!outline.hasTopEdge(of: rect))
    ...
    func degenerateRectCollapses() {
        let outline = Outline(
            XMenuBarBarBorderShape(cornerRadius: 8, omitTopEdge: true).path(in: .zero)
        )
    ...
    func insetsAccumulate() {
        let shape = XMenuBarBarBorderShape(cornerRadius: 15, omitTopEdge: false)
            .inset(by: 2)
            .inset(by: 3)
    ...
    func roundedClipGeometry() {
        let clip = XMenuBarBarBorderShape.xmenubarBarClip(height: 30, hasRoundedShape: true)
    ...
    func squareClipGeometry() {
        let clip = XMenuBarBarBorderShape.xmenubarBarClip(height: 30, hasRoundedShape: false)
    ...
    func borderInheritsClipGeometry(hasRoundedShape: Bool) {
        let clip = XMenuBarBarBorderShape.xmenubarBarClip(height: 30, hasRoundedShape: hasRoundedShape)
        let border = XMenuBarBarBorderShape.xmenubarBarBorder(
            height: 30,
    ...
    func onlySquareBorderOpensTopEdge() {
        #expect(
            XMenuBarBarBorderShape
                .xmenubarBarBorder(height: 30, hasRoundedShape: false, borderWidth: 1)
                .omitTopEdge
        )
        #expect(
            !XMenuBarBarBorderShape
                .xmenubarBarBorder(height: 30, hasRoundedShape: true, borderWidth: 1)
                .omitTopEdge
        )
    }
    ...
    func borderSitsInsideClip() {
        let clipBounds = Outline(
            XMenuBarBarBorderShape.xmenubarBarClip(height: 30, hasRoundedShape: true).path(in: rect)
        ).boundingRect
        let borderBounds = Outline(
            XMenuBarBarBorderShape
                .xmenubarBarBorder(height: 30, hasRoundedShape: true, borderWidth: 4)
                .path(in: rect)
        ).boundingRect
