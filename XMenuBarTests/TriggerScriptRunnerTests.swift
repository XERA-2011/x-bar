...
struct TriggerScriptRunnerTests {
    ...
    func runCapturesCombinedOutputWhenHostStdoutWasClosed() async throws {
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("xmenubar-trigger-script-closed-stdout-\(UUID().uuidString).sh")
    ...
    func timedOutScriptTerminatesDetachedDescendant() async throws {
        ...
        let markerURL = directory.appendingPathComponent("xmenubar-trigger-script-child-\(UUID().uuidString)")
        let scriptURL = directory.appendingPathComponent("xmenubar-trigger-script-timeout-\(UUID().uuidString).sh")
        defer {
    ...
    func timeoutEscalatesAfterDirectShellExitsToKillTermIgnoringDescendant() async throws {
        let directory = FileManager.default.temporaryDirectory
        let pidURL = directory.appendingPathComponent("xmenubar-trigger-script-child-pid-\(UUID().uuidString)")
        let scriptURL = directory.appendingPathComponent("xmenubar-trigger-script-term-ignore-\(UUID().uuidString).sh")
        defer {
    ...
    func detachedChildDoesNotHoldScriptResultOpen() async throws {
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("xmenubar-trigger-script-\(UUID().uuidString).sh")
        defer { try? FileManager.default.removeItem(at: scriptURL) }
    ...
    func expectedOutputAfterDiagnosticCapIsStreamMatched() async throws {
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("xmenubar-trigger-script-large-\(UUID().uuidString).sh")
        defer { try? FileManager.default.removeItem(at: scriptURL) }
    ...
    func unicodeExpectedOutputSplitAcrossPipeReadsIsStreamMatched() async throws {
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("xmenubar-trigger-script-unicode-\(UUID().uuidString).sh")
        defer { try? FileManager.default.removeItem(at: scriptURL) }
    ...
    func malformedByteBeforeUnicodeExpectedOutputIsStreamMatched() async throws {
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("xmenubar-trigger-script-malformed-\(UUID().uuidString).sh")
        defer { try? FileManager.default.removeItem(at: scriptURL) }
