...
/// Covers ``XMenuBarFocusFilter`` and ``ProfileEntityQuery``, the App Intents
/// surface that lets a macOS Focus mode switch XMenuBar's menu bar profile.
///
...
/// `"XMenuBar/Profiles/profiles.json"` and offers no injection point, unlike
/// `ProfileManager(profilesDirectory:)`. Its result therefore depends on
/// whether the machine running the tests has real XMenuBar profiles, and seeding
...
@MainActor
@Suite("XMenuBar Focus Filter intent", .serialized)
struct FocusFilterIntentTests {
    ...
    func performStoresTheSelectedProfileIdentifier() async throws {
        try await withScratchDefaults { suite in
            ...
            let filter = XMenuBarFocusFilter()
    ...
    func intentMetadata() throws {
        ...
        #expect(String(localized: XMenuBarFocusFilter.title) == String(localized: "Set Menu Bar Profile"))

        let description: IntentDescription = try #require(XMenuBarFocusFilter.description)
        #expect(
            String(localized: description.descriptionText)
                == String(localized: "Apply a XMenuBar menu bar profile when this Focus activates.")
        )
    ...
    func suggestedEntitiesAreIdentifiedByUUIDs() async throws {
        ...
        // what `XMenuBarFocusFilter.perform()` validates before storing it.
