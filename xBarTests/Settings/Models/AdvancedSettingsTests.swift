...
/// `xmenubar://` URL, so a key this model does not own, or a payload of the wrong
...
struct AdvancedSettingsTests {
    ...
    func storedValuesAreLoaded() throws {
        try withScratchDefaults { _ in
            ...
            Defaults.set(false, forKey: .useXMenuBarBarOnNotchOverflow)
            ...
            #expect(!settings.useXMenuBarBarOnNotchOverflow)
    ...
    func externalBooleanChangeIsApplied() async throws {
        try await withScratchDefaults { _ in
            ...
            await postExternalChanges([
                ...
                ["key": "useXMenuBarBarOnNotchOverflow", "value": false],
    ...
    func settingsWithoutAnExternalChannelAreLeftAlone() async throws {
        try await withScratchDefaults { _ in
            ...
            // This key is not handled by `handleExternalSettingsChange`, so a
            // well-formed payload for it must still change nothing.
            await postExternalChanges([
