...
///   map a `xmenubar://get?key=iceBarLocation` advertises after it silently
...
/// and writes go to a throwaway store rather than the real `com.xera.xmenubar`
...
struct SettingsURIHandlerGetTests {
    ...
    @Test("A callback URL without a usable scheme is refused", arguments: [
        ...
        "callback",
        "not a url at all",
        "/var/tmp/xmenubar-callback",
        "//example.com/xmenubar-callback",
        "://nope",
    ])
        ...
        try withScratchDefaults { _ in
            ...
            #expect(
                !SettingsURIHandler.handleGet(
                    key: "all",
                    displayUUID: nil,
                    callback: "file:///tmp/xmenubar-callback",
                    broadcast: true,
                    requestId: "req-both"
                )
            )
    ...
    func iceBarLocationGetListsAllValidValues() throws {
        try withScratchDefaults { _ in
            ...
            // could no longer be discovered through xmenubar://get. Drive the
