...
/// nothing here touches the developer's real `com.xera.xmenubar` domain. The
...
struct CoverageSweep3Tests {
    ...
    func urlForKeyResolvesAStoredPath() throws {
        try withScratchDefaults { suite in
            suite.set("/nonexistent/xmenubar-coverage-sweep/file.txt", forKey: Defaults.Key.menuBarSearchPanelFrame.rawValue)

            let url = try #require(Defaults.url(forKey: .menuBarSearchPanelFrame))

            #expect(url.path == "/nonexistent/xmenubar-coverage-sweep/file.txt")
    ...
    func globalDomainDoesNotSeeTheAppDomain() throws {
        try withScratchDefaults { _ in
            Defaults.set("xmenubar-coverage-sweep", forKey: .newItemsSection)

            #expect(Defaults.string(forKey: .newItemsSection) == "xmenubar-coverage-sweep")
