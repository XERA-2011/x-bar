...
/// Covers the geometry the xBar Bar's clip and border share, and the path
...
@Suite("xBar Bar border shape")
struct XBarBorderShapeTests {
    ...
    func keepingTopEdgeStaysClosed() {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 15, cornerStyle: .circular, omitTopEdge: false)
                .path(in: rect)
        )
    ...
    func omittingTopEdgeRemovesIt(style: RoundedCornerStyle) {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 7.5, cornerStyle: style, omitTopEdge: true)
                .path(in: rect)
        )
        #expect(!outline.isClosed)
        #expect(!outline.hasTopEdge(of: rect))
    ...
    func openEndsAreTopCorners() {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 7.5, cornerStyle: .continuous, omitTopEdge: true)
                .path(in: rect)
        )
    ...
    func zeroRadiusDrawsThreeEdges() {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 0, omitTopEdge: true).path(in: rect)
        )
    ...
    func insetPastRadiusDegenerates() {
        let shape = XBarBorderShape(cornerRadius: 4, omitTopEdge: true).inset(by: 6)
    ...
    func oversizedRadiusIsClamped() {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 500, cornerStyle: .circular, omitTopEdge: true)
                .path(in: rect)
        )
        #expect(outline.boundingRect.isNear(rect))
        #expect(!outline.hasTopEdge(of: rect))
    ...
    func degenerateRectCollapses() {
        let outline = Outline(
            XBarBorderShape(cornerRadius: 8, omitTopEdge: true).path(in: .zero)
        )
    ...
    func insetsAccumulate() {
        let shape = XBarBorderShape(cornerRadius: 15, omitTopEdge: false)
            .inset(by: 2)
            .inset(by: 3)
    ...
    func roundedClipGeometry() {
        let clip = XBarBorderShape.xbarBarClip(height: 30, hasRoundedShape: true)
    ...
    func squareClipGeometry() {
        let clip = XBarBorderShape.xbarBarClip(height: 30, hasRoundedShape: false)
    ...
    func borderInheritsClipGeometry(hasRoundedShape: Bool) {
        let clip = XBarBorderShape.xbarBarClip(height: 30, hasRoundedShape: hasRoundedShape)
        let border = XBarBorderShape.xbarBarBorder(
            height: 30,
    ...
    func onlySquareBorderOpensTopEdge() {
        #expect(
            XBarBorderShape
                .xbarBarBorder(height: 30, hasRoundedShape: false, borderWidth: 1)
                .omitTopEdge
        )
        #expect(
            !XBarBorderShape
                .xbarBarBorder(height: 30, hasRoundedShape: true, borderWidth: 1)
                .omitTopEdge
        )
    }
    ...
    func borderSitsInsideClip() {
        let clipBounds = Outline(
            XBarBorderShape.xbarBarClip(height: 30, hasRoundedShape: true).path(in: rect)
        ).boundingRect
        let borderBounds = Outline(
            XBarBorderShape
                .xbarBarBorder(height: 30, hasRoundedShape: true, borderWidth: 4)
                .path(in: rect)
        ).boundingRect
