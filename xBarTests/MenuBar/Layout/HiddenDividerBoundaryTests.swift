...
struct HiddenDividerBoundaryTests {
    ...
    struct Issue879FieldLog {
        ...
        static let currentVisible = [
            "com.xera.xmenubar:XMenuBar.ControlItem.Visible",
...
/// Pins the refusal that keeps the H_ctrl boundary move off XMenuBar's own
/// chevron (#958).
///
/// The candidate set the caller hands the planner is already filtered to
/// items that are movable and on screen. XMenuBar's control items pass both on
...
/// across with it.
///
/// #958's reporter imported a known-good plist with XMenuBar quit, confirmed it
...
/// Profile layout: moving H_ctrl -> left of <com.xera.xmenubar:XMenuBar.ControlItem.Visible>
...
@Suite("Boundary anchor refuses XMenuBar's own items")
struct BoundaryAnchorControlItemRefusalTests {
    private static let chevron = "com.xera.xmenubar:XMenuBar.ControlItem.Visible"
    private static let hiddenDivider = "com.xera.xmenubar:XMenuBar.ControlItem.Hidden"
