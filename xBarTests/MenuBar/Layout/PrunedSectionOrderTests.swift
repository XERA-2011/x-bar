...
struct PrunedSectionOrderTests {
    ...
    /// `Control Centre:XMenuBar.ControlItem.Visible` is a mis-tagged chevron;
    /// nothing live carries a control item title outside XMenuBar's namespace.
    @Test("A localized ghost of a XMenuBar control item is dropped")
    func localizedGhostOfControlItemIsDropped() {
        let ghost = "Control Centre:XMenuBar.ControlItem.Visible"
        let genuine = "com.xera.xmenubar:XMenuBar.ControlItem.Visible"
    ...
    /// Source-PID resolution handed Control Center's WiFi item XMenuBar's own PID,
    /// and the layout kept the result. Nothing live will carry that name.
    @Test("A foreign item saved under XMenuBar's namespace is dropped")
    ...
    /// XMenuBar's own items are the point of the namespace and must survive.
    @Test("XMenuBar's own control items and spacers survive")
    func keepsOwnControlItemsAndSpacers() {
        let own = Constants.bundleIdentifier
        let entries = [
            "\(own):XMenuBar.ControlItem.Visible",
            "\(own):XMenuBar.ControlItem.Hidden",
            "\(own):XMenuBar.ControlItem.AlwaysHidden",
            "\(own):XMenuBar.ControlItem.Visible.Spacer.0",
        ]
        let pruned = LayoutSolver.prunedSectionOrder(["visible": entries])
    ...
    func keepsIndexedControlItem() {
        let own = Constants.bundleIdentifier
        let pruned = LayoutSolver.prunedSectionOrder([
            "visible": ["\(own):XMenuBar.ControlItem.Visible:1"],
        ])
        #expect(pruned["visible"] == ["\(own):XMenuBar.ControlItem.Visible:1"])
    }
