...
/// file in the developer's own `~/Library/Logs/xBar`. `logDirectory`,
...
struct DiagnosticLoggerFileTests {
    ...
    @Test("The log directory is Library/Logs/xBar inside the user's home")
    func logDirectoryIsUnderTheUserLibrary() {
        let directory = DiagnosticLogger.shared.logDirectory

        #expect(directory.lastPathComponent == "xBar")
