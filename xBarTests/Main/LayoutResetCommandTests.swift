...
struct LayoutResetCommandTests {
    ...
    func flagSelectsTheCommand() {
        #expect(LayoutResetCommand.isRequested(arguments: ["/path/XMenuBar", "--reset-layout"]))
    }
    ...
    func flagIsFoundAmongOtherArguments() {
        #expect(LayoutResetCommand.isRequested(
            arguments: ["/path/XMenuBar", "-NSDocumentRevisionsDebugMode", "YES", "--reset-layout"]
        ))
    }

    @Test("A normal launch does not select the command")
    func normalLaunchDoesNotSelectTheCommand() {
        #expect(!LayoutResetCommand.isRequested(arguments: ["/path/XMenuBar"]))
    }
    ...
    func similarArgumentDoesNotSelectTheCommand() {
        #expect(!LayoutResetCommand.isRequested(arguments: ["/path/XMenuBar", "--reset-layout-please"]))
        #expect(!LayoutResetCommand.isRequested(arguments: ["/path/XMenuBar", "reset-layout"]))
    }
    ...
    func runIfRequestedResetsAndReports() {
        withDefaults { suite in
            ...
            let ran = LayoutResetCommand.runIfRequested(
                arguments: ["/path/XMenuBar", LayoutResetCommand.flag]
            )
    ...
    func normalLaunchLeavesLayoutUntouched() {
        withDefaults { suite in
            ...
            let ran = LayoutResetCommand.runIfRequested(arguments: ["/path/XMenuBar"])
