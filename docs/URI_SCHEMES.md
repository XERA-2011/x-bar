# XMenuBar URI Schemes & Deep Linking

XMenuBar supports custom URL schemes for deep linking, enabling integration with automation tools like Raycast, Alfred, and custom scripts.

## Overview

XMenuBar registers the `xmenubar://` URL scheme in `Info.plist` via `CFBundleURLTypes`. This allows external applications and scripts to trigger XMenuBar actions programmatically.
...
| --------------------------------- | --------------------- | ---------------------------------------- |
| `xmenubar://toggle-hidden`            | Toggle Hidden Section | Shows/hides the hidden menu bar section  |
| `xmenubar://toggle-always-hidden`     | Toggle Always-Hidden  | Shows/hides the always-hidden section    |
| `xmenubar://search`                   | Open Search Panel     | Displays the menu bar item search panel  |
| `xmenubar://toggle-xmenubarbar`           | Toggle XMenuBar Bar       | Toggles the XMenuBar Bar on the active display |
| `xmenubar://toggle-application-menus` | Toggle App Menus      | Shows/hides application menus            |
| `xmenubar://open-settings`            | Open Settings         | Opens the XMenuBar settings window           |
| `xmenubar://authorize`                | Authorize App         | Triggers auth dialog to grant an app whitelist access to settings |
...
```bash
open "xmenubar://toggle-hidden"
open "xmenubar://search"
open "xmenubar://open-settings"
```

#### Swift

```swift
NSWorkspace.shared.open(URL(string: "xmenubar://search")!)
```
...
tell application "System Events"
    open location "xmenubar://toggle-hidden"
end tell
```
...
# @raycast.title XMenuBar Actions
# @raycast.mode silent
...
The following URLs are configured in `XMenuBar/Resources/Info.plist` for internal use:

| Key                                   | Value                                 | Description                          |
| ------------------------------------- | ------------------------------------- | ------------------------------------ |
| `XMenuBarRepositoryURL`                   | `https://github.com/XERA-2011/x-menubar`    | GitHub repository                    |
| `XMenuBarDonateURL`                       | `https://github.com/sponsors/stonerl` | Sponsorship page                     |
| `XMenuBarMenuBarItemSpacingExecutableURI` | `file:///usr/bin/env`                 | Executable path for spacing commands |

## System URLs

XMenuBar uses the following system URLs to open macOS Settings:
...
XMenuBar supports programmatic settings manipulation via the `xmenubar://` URL scheme with a security whitelist. This allows automation tools like **Droppy** to control XMenuBar settings.
...
3. **First-Time Authorization**: New apps trigger a confirmation dialog with app name and permissions. Apps can proactively request authorization via `xmenubar://authorize` without reading or writing settings
...
| `useIceBarOnlyOnNotchedDisplay`           | Bool | XMenuBar Bar only on Macs with notch             |
...
| `useDoubleClickToShowAlwaysHiddenSection` | Bool | Double-click XMenuBar icon shows always-hidden   |
...
| `showIceIcon`                             | Bool | Show the XMenuBar icon in menu bar               |
| `iceBarLocationOnHotkey`                  | Bool | XMenuBar Bar appears at mouse location on hotkey     |
| `enableMenuBarItemOverflow`                | Bool | Eject items that would fall behind the notch |
| `useXMenuBarBarOnNotchOverflow`                | Bool | Send ejected overflow items to the XMenuBar Bar  |
...
| ------------------------ | ---- | ----- | ----------- |
| `useIceBar`              | Bool | Active display only | Enable/disable XMenuBar Bar on the display with the active menu bar |
| `useXMenuBarBarForAlwaysHidden` | Bool | All displays without IceBar | Open only the always-hidden section in the XMenuBar Bar, leaving hidden items to expand inline |
| `iceBarLocation`         | String | All displays with IceBar enabled | XMenuBar Bar position: `dynamic`, `mousePointer`, `iceIcon`, `leftAligned`, or `rightAligned` |
| `alwaysShowHiddenItems`  | Bool | All displays without IceBar | Show hidden items inline when IceBar is disabled |
| `iceBarLayout`           | String | All displays with IceBar enabled | XMenuBar Bar layout: `horizontal`, `vertical`, or `grid` |
...
- `useXMenuBarBarForAlwaysHidden`: Updates all displays that do NOT have the IceBar enabled
...
```text
xmenubar://set?key=<setting>&value=<true|false>
```
...
```bash
# Enable auto-rehide
open "xmenubar://set?key=autoRehide&value=true"

# Disable hover reveal
open "xmenubar://set?key=showOnHover&value=false"

# Enable XMenuBar Bar
open "xmenubar://set?key=useIceBar&value=true"
```

#### Toggle a Boolean Value

```text
xmenubar://toggle?key=<setting>
```
...
open "xmenubar://toggle?key=autoRehide"

# Toggle XMenuBar Bar visibility (active display only)
open "xmenubar://toggle?key=useIceBar"

# Toggle application menu hiding
open "xmenubar://toggle?key=hideApplicationMenus"

# Set IceBar location (all displays with IceBar enabled)
open "xmenubar://set?key=iceBarLocation&value=mousePointer"

# Set IceBar aligned left (all displays with IceBar enabled)
open "xmenubar://set?key=iceBarLocation&value=leftAligned"

# Set IceBar aligned right (all displays with IceBar enabled)
open "xmenubar://set?key=iceBarLocation&value=rightAligned"

# Enable always-show-hidden-items (all displays without IceBar)
open "xmenubar://set?key=alwaysShowHiddenItems&value=true"

# Set XMenuBar Bar layout to grid (all displays with IceBar enabled)
open "xmenubar://set?key=iceBarLayout&value=grid"

# Set grid columns to 5 (all displays with IceBar enabled)
open "xmenubar://set?key=gridColumns&value=5"

# Set rehide interval to 10 seconds (clamped to range 1-300)
open "xmenubar://set?key=rehideInterval&value=10"

# Set hover delay to 0.5 seconds
open "xmenubar://set?key=showOnHoverDelay&value=0.5"

# Set rehide strategy to "timed" (0=smart, 1=timed, 2=focusedApp)
open "xmenubar://set?key=rehideStrategy&value=timed"
# Or using numeric value
open "xmenubar://set?key=rehideStrategy&value=1"
```
...
```bash
# Enable XMenuBar Bar on specific display by UUID
open "xmenubar://set?key=useIceBar&value=true&display=37D8832A-2D66-02CA-B9F7-8F30A301B230"

# Set IceBar location on specific display
open "xmenubar://set?key=iceBarLocation&value=iceIcon&display=ABC12345-..."

# Toggle XMenuBar Bar on specific display
open "xmenubar://toggle?key=useIceBar&display=XYZ789-..."
```
...
External apps can proactively request authorization via `xmenubar://authorize`. This triggers the macOS permission dialog for the calling app without needing to read or write any settings.

```bash
# Request whitelist authorization for the calling app
open "xmenubar://authorize"
```
...
open "xmenubar://authorize"
open "xmenubar://get?key=all&callback=myapp://response&requestId=1"
```

### Getting Settings (Read Operations)

XMenuBar supports reading settings via `xmenubar://get` URLs. You must provide a response mechanism: either a `callback` URL (recommended) or `broadcast=true` for acknowledgement notifications.
...
open "xmenubar://get?key=all&callback=droppy://xmenubar-response&requestId=abc123"
```
  ...
  "data": {
    ...
    "displays": {
      "37D8832A-2D66-02CA-B9F7-8F30A301B230": {
        ...
        "useIceBar": true,
        "useXMenuBarBarForAlwaysHidden": false,
...
```bash
# Get single setting
open "xmenubar://get?key=autoRehide&callback=droppy://xmenubar-response"

# Get per-display setting
open "xmenubar://get?key=useIceBar&display=37D8832A-...&callback=droppy://xmenubar-response"
```
...
open "xmenubar://get?key=version&callback=droppy://xmenubar-response&requestId=abc123"

# Or via broadcast
open "xmenubar://get?key=version&broadcast=true&requestId=abc123"
```
...
```bash
# Get all displays
open "xmenubar://get?key=displays&callback=droppy://xmenubar-response"

# Get specific display
open "xmenubar://get?key=display&display=37D8832A-...&callback=droppy://xmenubar-response"
```
...
- XMenuBar opens the provided URL with URL-encoded JSON data
...
- XMenuBar broadcasts via `DistributedNotificationCenter`
...
open "xmenubar://get?key=all&broadcast=true&requestId=abc123"
```
...
open "xmenubar://set?key=showOnHover&value=true&bundleId=com.apple.Terminal"
...
# @raycast.schemaVersion 1
# @raycast.title Toggle XMenuBar Setting
# @raycast.mode silent
# @raycast.argument1 { "type": "dropdown", "placeholder": "Setting", "data": [{"title": "Auto-Rehide", "value": "autoRehide"}, {"title": "Hover Reveal", "value": "showOnHover"}, {"title": "XMenuBar Bar", "value": "useIceBar"}] }

open "xmenubar://toggle?key=${1}"
```
...
- Test with XMenuBar itself (DEBUG builds only)
...
Check XMenuBar's diagnostic logs for details on failed requests.

## Notes

- All `xmenubar://` URLs work even when XMenuBar is not currently in the foreground
