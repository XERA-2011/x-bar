# Verifying XMenuBar releases

XMenuBar ships macOS app builds intended for wide use. Releases are protected by
...
| --- | --- | --- |
| **Apple Developer ID + notarization** | Binary came from the XMenuBar developer team and passed Apple’s notarization checks | Release GitHub Actions (signing + `notarytool`) |
...
| **CycloneDX SBOM** | Machine-readable inventory of the SwiftPM dependencies linked into the shipped app | Syft (pinned by version and archive digest) over the tagged source tree in the release workflow; uploaded as `XMenuBar_<tag>.cdx.json` |
...
The EdDSA public key shipped inside XMenuBar:

```text
fQ2kWqCLfAPAxQX1rp8gVNoG9hlAV/Gmm7kMBbxFe+A=
```

Source of truth in-tree: `XMenuBar/Resources/Info.plist` key `SUPublicEDKey`.  
Appcast URL: `https://xmenubar-app.github.io/updates/appcast.xml` (`SUFeedURL`).
Legacy installs may still poll `https://stonerl.github.io/XMenuBar/appcast.xml`;
...
spctl --assess --type execute -v /path/to/XMenuBar.app

# Notarization ticket staple / history (when available)
xcrun stapler validate /path/to/XMenuBar.app
codesign --verify --deep --strict --verbose=2 /path/to/XMenuBar.app
codesign -dv --verbose=4 /path/to/XMenuBar.app 2>&1 | grep -E 'Authority|TeamIdentifier|Timestamp'
```
...
Release installers on `XERA-2011/x-menubar` include a companion
`XMenuBar.dmg.sigstore.json` (or similarly named) asset produced by cosign keyless
signing in CI, plus a `XMenuBar.dmg.sha256` checksum.

The identity pattern below is anchored to the release workflow on purpose: a
loose pattern such as `https://github.com/XERA-2011/x-menubar/` would also accept a
...
shasum -a 256 -c XMenuBar.dmg.sha256

cosign verify-blob \
  --bundle XMenuBar.dmg.sigstore.json \
  --certificate-identity-regexp '^https://github\.com/XERA-2011/x-menubar/\.github/workflows/release\.yml@refs/' \
...
signer. Moving the build into [`xmenubar-app/org-ci`](https://github.com/xmenubar-app/org-ci)
is the prerequisite for that claim.

The same attestation bundle is attached to the GitHub Release as
`XMenuBar.dmg.intoto.jsonl` / `XMenuBar_<tag>.cdx.json.intoto.jsonl` (for offline copy
and OpenSSF Scorecard Signed-Releases). Prefer verifying via the Attestation
Store:

```sh
# After downloading XMenuBar.dmg from the GitHub Release:
gh attestation verify XMenuBar.dmg --repo XERA-2011/x-menubar \
  --signer-workflow XERA-2011/x-menubar/.github/workflows/attest-build-provenance.yml

# Same for the SBOM:
gh attestation verify XMenuBar_2.0.0.cdx.json --repo XERA-2011/x-menubar \
  --signer-workflow XERA-2011/x-menubar/.github/workflows/attest-build-provenance.yml
```
...
Each XMenuBar GitHub Release that went through the current release workflow includes:

| Asset | Purpose |
| --- | --- |
| `XMenuBar_<tag>.cdx.json` | CycloneDX Software Bill of Materials for the release's resolved SwiftPM dependencies |
| `XMenuBar_<tag>.cdx.json.sha256` | SHA-256 of the SBOM file |
| `XMenuBar_<tag>.cdx.json.sigstore.json` | Cosign keyless signature bundle for the SBOM |
| `XMenuBar_<tag>.cdx.json.intoto.jsonl` | SLSA build provenance attestation bundle (also in the GitHub Attestation Store) |

```sh
# After downloading the three files from the GitHub Release:
shasum -a 256 -c XMenuBar_2.0.0.cdx.json.sha256

cosign verify-blob \
  --bundle XMenuBar_2.0.0.cdx.json.sigstore.json \
  --certificate-identity-regexp '^https://github\.com/XERA-2011/x-menubar/\.github/workflows/release\.yml@refs/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  XMenuBar_2.0.0.cdx.json
```
...
- **GitHub Releases:** https://github.com/XERA-2011/x-menubar/releases  
