# XMenuBar security assurance case

This document argues that XMenuBar’s [security requirements](../.github/SECURITY.md)
...
architecture). XMenuBar holds the OpenSSF Best Practices **Gold** badge, with the
...
1. XMenuBar runs locally; no XMenuBar account and no first-party tracking backend.
...
3. Automation that **mutates settings** via `xmenubar://` requires an explicit
...
2. A hardened multi-tenant server security model (XMenuBar is not a network service).
...
| TCC permissions held by XMenuBar | AX / screen capture are powerful on macOS |

### Adversaries / scenarios

| ID | Scenario | Likelihood | Impact |
| --- | --- | --- | --- |
| T1 | Malicious local app sends `xmenubar://` settings URLs | Medium | Config abuse, UX disruption |
| T2 | Supply-chain compromise of a SwiftPM dependency or CI | Low–Med | Code execution in XMenuBar |
...
| T5 | Confused-deputy via Accessibility APIs | Inherent | XMenuBar can affect other apps’ status items by design |
...
| --- | --- | --- |
| External URL → settings | `xmenubar://set` etc. | Allowlisted keys; range checks; sender whitelist + `SecCode` Team ID binding |
...
| Principle | How XMenuBar applies it |
| --- | --- |
...
1. Accessibility by design can manipulate other apps’ menu bar items, so users
   must trust XMenuBar similarly to other AX utilities.
2. Private/undocumented WindowServer interactions increase compatibility and
   ...
   leaks a small dictionary in the caller. That contains growth in XMenuBar but does
...
3. Update delivery depends on a GitHub Pages path. The Sparkle appcast is
   ...
   [`xmenubar-app`](https://github.com/xmenubar-app) (three owners: `stonerl`,
...
4. **Statement coverage (`test_statement_coverage80`) is Met against a
   documented exclusion set.** Statement coverage for `xmenubar-app_XMenuBar` is
...
| URI allowlist + signature binding | `XMenuBar/Utilities/SettingsURIHandler.swift`, `docs/URI_SCHEMES.md` |
| Update authenticity | `SUFeedURL` / `SUPublicEDKey` in `XMenuBar/Resources/Info.plist`; Sparkle release actions |
| Architecture | `docs/ARCHITECTURE.md` |
| Automated analysis | `.github/workflows/ci.yml`, SonarCloud project `xmenubar-app_XMenuBar` |
...
| 2026-07-31 | Claimed Silver `test_statement_coverage80`; measured 80.8% after migrating the suite to Swift Testing and documenting the exclusion set. Corrected the SonarCloud project key to `xmenubar-app_XMenuBar` |
